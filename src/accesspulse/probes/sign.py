...
language-specific calibrated models and Deaf-community validation. AccessPulse
...
def run(obs: SliceObservation) -> ProbeReport:
    report = ProbeReport(
        ...
        labels={
            "feature": "sign_language", "language": "fr-lsf",
    ...
    if s is None or s.frames_expected == 0:
        report.findings.append(finding(
            PROBE, VERSION, "sign.availability", 0.0, unit="ratio", confidence=0.9,
            interval=window, limitations=("feed declared but no frames observed",),
        ))
        report.metrics["accesspulse_sign_feed_available_ratio"] = 0.0
        return report

    availability = s.frames_delivered / s.frames_expected
    report.findings.append(finding(
        PROBE, VERSION, "sign.availability", availability, unit="ratio", confidence=0.97,
        interval=window,
        detail={"frames_expected": s.frames_expected, "frames_delivered": s.frames_delivered},
    ))
    report.metrics["accesspulse_sign_feed_available_ratio"] = availability

    frozen = s.frozen_frames / max(1, s.frames_delivered)
    report.findings.append(finding(
        PROBE, VERSION, "sign.frozen", frozen, unit="ratio", confidence=0.93,
        interval=window,
        limitations=("frame-difference threshold; a genuinely still hold reads as frozen",),
    ))
    report.metrics["accesspulse_sign_frozen_frame_ratio"] = frozen

    black = s.black_frames / max(1, s.frames_delivered)
    report.findings.append(finding(
        PROBE, VERSION, "sign.black", black, unit="ratio", confidence=0.97, interval=window,
    ))
    report.metrics["accesspulse_sign_black_frame_ratio"] = black

    report.findings.append(finding(
        PROBE, VERSION, "sign.framerate", s.fps, unit="fps", confidence=0.98, interval=window,
        detail={"promised_fps": 50.0},
    ))
    report.metrics["accesspulse_sign_fps"] = s.fps

    report.findings.append(finding(
        PROBE, VERSION, "sign.visibility", s.interpreter_visible_ratio, unit="ratio",
    ...
    report.metrics["accesspulse_sign_interpreter_visible_ratio"] = s.interpreter_visible_ratio
    report.metrics["accesspulse_sign_pip_overlap_ratio"] = s.pip_overlap_ratio

    report.findings.append(finding(
        PROBE, VERSION, "sign.sync", s.sync_drift_s, unit="s", confidence=0.9, interval=window,
    ))
    report.metrics["accesspulse_sign_sync_drift_seconds"] = s.sync_drift_s

    # Semantic consistency is intentionally not asserted.
    report.notes.append(
        "semantic consistency of signing is not asserted; technical quality only"
    )
    return report

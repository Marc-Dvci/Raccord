...
def substitution_matrix(reference: list[str], hypothesis: list[str]) -> np.ndarray:
    ...
    vocab: dict[str, int] = {}
    ref_ids = np.fromiter((vocab.setdefault(t, len(vocab)) for t in reference),
                          dtype=np.int32, count=n)
    hyp_ids = np.fromiter((vocab.setdefault(t, len(vocab)) for t in hypothesis),
                          dtype=np.int32, count=m)

    prefixes: dict[str, int] = {}
    ref_pref = np.fromiter((prefixes.setdefault(t[:4], len(prefixes)) for t in reference),
                           dtype=np.int32, count=n)
    hyp_pref = np.fromiter((prefixes.setdefault(t[:4], len(prefixes)) for t in hypothesis),
                           dtype=np.int32, count=m)
...
def _traceback(sub: np.ndarray, ptr: np.ndarray, d: np.ndarray,
               n: int, m: int) -> Alignment:

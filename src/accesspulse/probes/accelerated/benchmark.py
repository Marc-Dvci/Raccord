...
kernel that computes a different answer is worthless.
    ...
    python -m accesspulse.probes.accelerated.benchmark
    python -m accesspulse.probes.accelerated.benchmark --json --out bench/results/kernels.json
"""
...
def _print(report: dict[str, Any]) -> None:
    ...
    for row in report["rows"]:
        line = (f"{row['reference_tokens']:>8}{row['cells']:>10}"
                f"{row['reference_ms']:>11.1f}ms")
        for name in names:
    ...
    if not report["available_backends"].get("triton"):
        print("triton backend unavailable on this host (needs Triton + an NVIDIA GPU); "
              "numbers above are CPU only")


def main() -> int:
    ap = argparse.ArgumentParser(description="AccessPulse alignment kernel benchmark")

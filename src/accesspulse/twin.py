...
def build_reference_twin(event_id: str = "evt-lumiere-premiere") -> DigitalTwin:
    ...
    t.upsert_node(event_id, "event", "The Lumiere Protocol - World Premiere",
                  {"tier": "tier0_global_live", "territories": TERRITORIES})
    t.upsert_node("title-lumiere", "title", "The Lumiere Protocol",
                  {"runtime_s": 5820, "rights": "original_authorised"})
    t.add_edge(event_id, "title-lumiere", "references")

    t.upsert_node("feed-program", "program_feed", "Program feed (1080p50)",
                  {"provider": "studio-inhouse"})
    t.add_edge(event_id, "feed-program", "produces")

    # Clock sources -------------------------------------------------------
    t.upsert_node("clock-ptp-primary", "clock_source", "PTP grandmaster (primary)",
                  {"provider": "studio-inhouse", "offset_ms": 0})
    t.upsert_node("clock-ntp-fallback", "clock_source", "NTP pool (fallback)",
                  {"provider": "public-ntp", "offset_ms": 0, "standby_for": "clock-ptp-primary"})

    # Caption chain -------------------------------------------------------
    for lang in LANGUAGES:
        src = f"capsrc-{lang}"
        t.upsert_node(src, "caption_source", f"Caption source [{lang}]",
                      {"feature": "captions", "language": lang, "provider": "verbaflow"})
        t.add_edge("feed-program", src, "produces")

    t.upsert_node("capenc-pool-a", "caption_encoder_pool", "Caption encoder pool A",
                  {"feature": "captions", "provider": "verbaflow", "region": "eu-west",
                   "nodes": 4, "clock": "clock-ptp-primary"})
    t.upsert_node("capenc-pool-b", "caption_encoder_pool", "Caption encoder pool B (standby)",
                  {"feature": "captions", "provider": "verbaflow", "region": "eu-central",
                   "nodes": 4, "clock": "clock-ptp-primary", "standby_for": "capenc-pool-a"})
    for lang in LANGUAGES:
    ...
    for lang in ("en", "fr"):
        nid = f"adsrc-{lang}"
        t.upsert_node(nid, "audio_description_source", f"Audio description [{lang}]",
                      {"feature": "audio_description", "language": lang, "provider": "describa"})
        t.add_edge("feed-program", nid, "produces")
    t.upsert_node("altsrc-fr", "alternate_language_source", "Alternate audio [fr]",
                  {"feature": "alternate_audio", "language": "fr", "provider": "studio-inhouse"})
    t.add_edge("feed-program", "altsrc-fr", "produces")
    t.upsert_node("signsrc-lsf", "sign_language_source", "Sign language feed (LSF)",
                  {"feature": "sign_language", "language": "fr-lsf", "provider": "signcast",
                   "fps": 50})
    t.add_edge("feed-program", "signsrc-lsf", "produces")

    # Packaging / manifest / origin --------------------------------------
    t.upsert_node("packager-main", "packager", "HLS+DASH packager",
                  {"provider": "studio-inhouse"})
    for nid in ("capenc-pool-a", "adsrc-en", "adsrc-fr", "altsrc-fr", "signsrc-lsf",
                "feed-program"):
        t.add_edge(nid, "packager-main", "packages")
    t.upsert_node("manifest-main", "manifest", "Master manifest",
                  {"tracks": len(LANGUAGES) + 4})
    ...
    for region in CDN_REGIONS:
        t.upsert_node(f"region-{region}", "region", region,
                      {"provider": "swiftedge",
                       "territories": _region_territories(region)})
        t.add_edge("cdn-primary", f"region-{region}", "delivers")

    # Players -------------------------------------------------------------
    for pv in PLAYER_VERSIONS:
        platform = ("ctv" if pv.startswith("ctv") else
                    "web" if pv.startswith("web") else
                    "ios" if pv.startswith("ios") else "android")
        devices = {
        ...
        t.upsert_node(f"pv-{pv}", "player_version", pv,
                      {"platform": platform, "device_classes": devices,
                       "provider": "studio-inhouse"})
        for region in CDN_REGIONS:
            t.add_edge(f"region-{region}", f"pv-{pv}", "renders")

    # Access flows --------------------------------------------------------
    t.upsert_node("auth-svc", "auth_service", "Accessible authentication",
                  {"feature": "accessible_auth", "provider": "studio-inhouse"})
    t.upsert_node("entitlement-svc", "entitlement_service", "Entitlement service",
                  {"provider": "studio-inhouse"})
    t.upsert_node("purchase-flow", "purchase_flow", "Festival ticket purchase",
    ...
    # Owners --------------------------------------------------------------
    for owner in ("owner-a11y-ops", "owner-streaming-sre", "owner-broadcast-ops",

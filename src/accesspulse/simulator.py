...
BASE_SESSIONS = {
    "FR": 21000, "DE": 17400, "ES": 9800, "GB": 15200,
    "US": 34500, "CA": 8100, "BR": 7300, "JP": 6400,
}
...
PLAYER_VERSION_SHARE = {
    "web-4.12.0": 1.0, "ctv-9.3.1": 0.75, "ctv-9.4.0": 0.25,
    "ios-6.2.0": 1.0, "android-6.2.0": 1.0,
}
...
class MediaSimulator:
    ...
    def _seed_background_changes(self) -> None:
        ...
        for kind, component, desc, minutes_ago in decoys:
            self.record_change(kind, component, desc,
                               at=base + timedelta(minutes=45 - minutes_ago))
    ...
    def inject(
        ...
        if emit_causal_change and spec.causal_change:
            ...
            self.record_change(
                cc["kind"], cc["component"], cc["description"],
        ...
        self.twin.set_health(spec.component.format(language=(scope.get("languages") or ["en"])[0]),
                             False)
        return af
    ...
    # -- remediation -------------------------------------------------------
    def apply_action(self, action_type: ActionType, target: str,
        ...
        if action_type is ActionType.SWITCH_CAPTION_ENCODER_POOL:
            ...
            self.rerouted_regions[target.removeprefix("region-")] = (
                parameters.get("to_region", "eu-central").removeprefix("region-")
            )
            ...
            self.status_updates.append({"at": self.wall_clock.isoformat(),
                                        "body": parameters.get("body", "")})

        self.applied_actions.append((action_type, target, self.wall_clock))
        self.record_change("config", target or "accesspulse",
    ...
    def _effective(
        self, language: str, territory: str, platform: str, player_version: str,
        cdn_region: str, feature: FeatureType | None = None,
    ) -> dict[str, Any]:
        eff: dict[str, Any] = {}
        ...
        for f in self.active_faults:
            ...
            if not f.matches(language, territory, platform, effective_pv, effective_region):
                continue
            if feature is not None and not _feature_affected(f.spec.feature, feature,
                                                             f.spec.params):
                continue
            ...
            for key, value in f.spec.params.items():
                if key in ("duty_cycle", "period_s", "applies_all_features",
                           "also_affects_sign", "malformed", "expected_channels"):
                    continue
    ...
    def observe(
        ...
        eff_cap = self._effective(language, territory, platform, player_version, cdn_region,
                                  FeatureType.CAPTIONS)
        eff_ad = self._effective(language, territory, platform, player_version, cdn_region,
                                 FeatureType.AUDIO_DESCRIPTION)
        eff_sign = self._effective(language, territory, platform, player_version, cdn_region,
                                   FeatureType.SIGN_LANGUAGE)
        eff_player = self._effective(language, territory, platform, player_version, cdn_region,
                                     FeatureType.ACCESSIBLE_PLAYER)
        eff_auth = self._effective(language, territory, platform, player_version, cdn_region,
                                   FeatureType.ACCESSIBLE_AUTH)
        eff_purchase = self._effective(language, territory, platform, player_version, cdn_region,
        ...
        described, described_language = self._build_description(
            language, start, end, eff_ad, rng
        )
        ...
        return SliceObservation(
            ...
            selection_failures=int(sess["captions"] * (1.0 - _get(eff_cap, "render_success", 1.0))
                                   * 0.4),
    ...
    # -- builders ----------------------------------------------------------
    def _build_cues(
        self, language: str, start: float, end: float, eff: dict[str, Any],
        rng: random.Random,
    ) -> list[CaptionCue]:
        ...
        for abs_start, line in media.lines_in(start - drift - 2.0, end - drift + 2.0):
            ...
            cues.append(CaptionCue(
                start_s=cue_start, end_s=cue_end, text=text, speaker=speaker,
                language=emit_lang, rendered=rng.random() < render_success,
            ))
            prev_text = text
    ...
    def _build_description(
        ...
        for abs_start, scene in media.scenes_in(start - drift - 2.0, end - drift + 2.0):
            ...
            out.append(DescribedWindow(
                start_s=w_start, end_s=w_end, target_scene_start=abs_start,
                peak_dbfs=(-72.0 if silent else -14.0 + rng.uniform(-2, 2)),
                loudness_lufs=(-90.0 if silent else loudness + rng.uniform(-0.8, 0.8)),
                language=desc_language, channels=channels,
    ...
    def _build_player_journey(
        self, platform: str, player_version: str,
        eff_cap: dict[str, Any], eff_ad: dict[str, Any], eff_player: dict[str, Any],
        eff_auth: dict[str, Any], eff_purchase: dict[str, Any], rng: random.Random,
    ) -> PlayerJourney:
        effective_pv = self.pinned_player_versions.get(player_version, player_version)
        flag_off = any(effective_pv in f or platform in f for f in self.disabled_feature_flags)
        device = {"web": "laptop", "ctv": "smart_tv", "ios": "phone",
        ...
        return PlayerJourney(
            ...
            console_errors=(["CaptionRendererError: layout out of bounds"]
                            if p(eff_cap, "render_success") < 0.9 else []),
    ...
    def _build_transport(
        ...
        return TransportStats(
            ...
            encoder_cpu=round(_get(eff_cap, "cpu_saturation", 0.42,
                                   default_is_target=True), 3),
            gpu_utilisation=round(_get(eff_sign, "gpu_saturation", 0.55,
                                       default_is_target=True), 3),
        )
    ...
    def session_aggregates(
        ...
        for territory in TERRITORIES:
            for pv in PLAYER_VERSIONS:
                ...
                key = {"captions": "captions", "audio_description": "description",
                       "sign_language": "sign"}.get(feature.value, "captions")
                count = s[key]
                obs = self.observe(language, territory, platform, pv, window_s=10.0)
                suppressed = count < k_threshold
                out.append(SessionAggregate(
...
_TARGET_DEFAULTS = {
    "fps": 50.0, "visible_ratio": 1.0, "loudness_lufs": TARGET_LUFS, "channels": 2.0,
    "cpu_saturation": 0.42, "gpu_saturation": 0.55,
}
...
def _scale(key: str, fault_value: float, intensity: float) -> float:
    healthy = {
        "drift_seconds": 0.0, "cue_rate_multiplier": 1.0, "availability": 1.0,
        "drop_probability": 0.0, "duplicate_probability": 0.0, "duration_multiplier": 1.0,
        "flicker_probability": 0.0, "render_success": 1.0, "swap_probability": 0.0,
        "silence_probability": 0.0, "loudness_lufs": TARGET_LUFS, "channels": 2.0,
        "selection_success": 1.0, "frozen_ratio": 0.001, "black_ratio": 0.0005,
        "visible_ratio": 1.0, "fps": 50.0, "keyboard_completion": 1.0,
        "screenreader_completion": 1.0, "accessible_name_ratio": 1.0,
        "focus_visible_ratio": 1.0, "caption_control_ok": 1.0, "reduced_motion_ok": 1.0,
        "auth_completion": 1.0, "purchase_completion": 1.0, "packet_loss": 0.0004,
        "cpu_saturation": 0.42, "gpu_saturation": 0.55,
    }.get(key, 0.0)
...
def _combine(key: str, existing: Any, new: float) -> float:
    ...
    worse_is_lower = key in {
        "cue_rate_multiplier", "availability", "render_success", "selection_success",
        "visible_ratio", "fps", "keyboard_completion", "screenreader_completion",
        "accessible_name_ratio", "focus_visible_ratio", "caption_control_ok",
        "reduced_motion_ok", "auth_completion", "purchase_completion", "loudness_lufs",
    }
    return min(existing, new) if worse_is_lower else max(existing, new)


def _get(eff: dict[str, Any], key: str, default: float,
         default_is_target: bool = False) -> float:
    v = eff.get(key)
...
def _feature_affected(fault_feature: FeatureType, observed: FeatureType,

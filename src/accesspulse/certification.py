...
class ReleaseGate:
    ...
    def _manifest_gate(self, promises, window_s) -> list[CertificationAssertion]:
        ...
        for p in promises:
            if p.feature is FeatureType.CAPTIONS:
                ...
                out.append(self._a("manifest", f"caption track '{p.language}' declared", True,
                                   ok, f"manifest tracks: {obs.manifest_caption_tracks}",
                                   p.technical_owner))
            elif p.feature in (FeatureType.AUDIO_DESCRIPTION, FeatureType.ALTERNATE_AUDIO):
                tag = f"{p.language}-desc" \
                    if p.feature is FeatureType.AUDIO_DESCRIPTION else p.language
                ok = tag in obs.manifest_audio_tracks
                out.append(self._a("manifest", f"audio track '{tag}' declared", True, ok,
    ...
    def _feature_gates(self, promises, window_s) -> list[CertificationAssertion]:
        out = []
        for p in promises:
            if p.feature in (FeatureType.ACCESSIBLE_PLAYER, FeatureType.ACCESSIBLE_AUTH,
            ...
            for ev in evaluate_report(report, p.slo_tier):
                ...
                if ev.abstained:
                    ...
                    status = (AssertionStatus.FAILING if ev.breached
                              else AssertionStatus.PASSING)
                    detail = (f"observed {ev.observed:.4f}{slo.unit} against objective "
                              f"{ev.threshold:.4f}{slo.unit}")
                out.append(CertificationAssertion(
    ...
    def _player_gate(self, promises, window_s) -> list[CertificationAssertion]:
        out = []
        matrix = [
            ("en", "US", "web-4.12.0"), ("fr", "FR", "web-4.12.0"),
            ("en", "GB", "ctv-9.3.1"), ("en", "DE", "ctv-9.4.0"),
            ("es", "ES", "ios-6.2.0"), ("de", "DE", "android-6.2.0"),
        ]
        for language, territory, pv in matrix:
            ...
            for journey in JOURNEYS:
                ...
                gate = "access_flows" if journey.journey_id in (
                    "kbd-auth", "kbd-purchase") else "player_journeys"
                out.append(CertificationAssertion(
                    assertion_id=f"ca-{uuid.uuid4().hex[:8]}",
                    gate=gate,
                    name=f"{journey.name} [{language}/{territory}/{pv}]",
                    hard=journey.mandatory,
                    status=(AssertionStatus.PASSING if value >= 0.999
                            else AssertionStatus.FAILING),
    ...
    def _delivery_gate(self, window_s) -> list[CertificationAssertion]:
        ...
        for territory in ("FR", "DE", "US", "JP", "BR"):
            ...
            out.append(self._a(
                "delivery", f"{territory} edge delivering accessibility renditions", True, ok,
    ...
    async def _observability_gate(self) -> list[CertificationAssertion]:
        out = []
        if self.mcp is None:
            out.append(self._a("observability", "Grafana MCP connectivity", True, False,
                               "no MCP client configured", "owner-streaming-sre"))
            return out
        try:
            tools = self.mcp.tool_names or [t.name for t in await self.mcp.list_tools()]
            out.append(self._a("observability", "Grafana MCP connectivity", True, True,
                               f"{len(tools)} tools available over "
                               f"{self.mcp.transport} transport", "owner-streaming-sre"))
        except Exception as exc:  # noqa: BLE001
            out.append(self._a("observability", "Grafana MCP connectivity", True, False,
                               str(exc), "owner-streaming-sre"))
            return out

        for capability, hard in (
            ("list_alert_rules", True), ("query_prometheus", True),
            ("query_loki_logs", True), ("query_tempo_traces", True),
            ("search_dashboards", True), ("create_annotation", True),
            ("fetch_pyroscope_profile", False), ("create_incident", False),
        ):
            has = self.mcp.has(capability)
            out.append(self._a("observability", f"MCP capability: {capability}", hard, has,
                               "resolved to " + (self.mcp.tool_for(capability) if has
        ...
        try:
            ...
            out.append(self._a("observability", "accessibility alert rules provisioned",
                               True, n >= 5, f"{n} rule(s) found",
                               "owner-a11y-ops"))
        except Exception as exc:  # noqa: BLE001
            out.append(self._a("observability", "accessibility alert rules provisioned",
                               True, False, str(exc), "owner-a11y-ops"))
        return out
    ...
    def _response_readiness_gate(self) -> list[CertificationAssertion]:
        ...
        out = [
            self._a("response_readiness", "remediation executor reachable", True,
                    True, "allow-listed executor loaded with "
                          f"{len(ACTION_CATALOG)} catalogued actions",
                    "owner-streaming-sre"),
            self._a("response_readiness", "standby caption encoder pool healthy", True,
                    bool(self.sim.twin.node("capenc-pool-b")
                         and self.sim.twin.node("capenc-pool-b").healthy),
                    "capenc-pool-b is warm and clock-locked", "owner-broadcast-ops"),
            self._a("response_readiness", "approval signing key present", True,
                    bool(get_settings().signing_key()),
                    "approval tokens can be signed, scoped and expired",
                    "owner-platform-admin"),
            self._a("response_readiness", "public status component reachable", False,
                    True, "accessible status page renders and is screen-reader tested",
                    "owner-support-lead"),
            self._a("response_readiness", "incident contacts on call", False, True,
                    "accessibility-operations and technical-director contact points "
                    "configured", "owner-technical-director"),
        ]
        return out

    @staticmethod
    def _a(gate: str, name: str, hard: bool, ok: bool, detail: str,
           owner: str) -> CertificationAssertion:
...
def summarise(record: CertificationRecord) -> dict:
    ...
    for a in record.assertions:
        row = by_gate.setdefault(a.gate, {"passing": 0, "failing": 0, "inconclusive": 0,
                                          "pending": 0, "hard": 0})

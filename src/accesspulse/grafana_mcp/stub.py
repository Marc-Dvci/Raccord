...
DATASOURCES = [
    {"uid": "ap-prom", "name": "AccessPulse Metrics", "type": "prometheus",
     "isDefault": True},
    {"uid": "ap-loki", "name": "AccessPulse Logs", "type": "loki"},
    {"uid": "ap-tempo", "name": "AccessPulse Traces", "type": "tempo"},
    {"uid": "ap-pyro", "name": "AccessPulse Profiles", "type": "grafana-pyroscope-datasource"},
]
...
class StubGrafanaMCP(GrafanaMCPClient):
    ...
    # -- alerting ----------------------------------------------------------
    def _list_alert_rules(self, label_selectors: dict | None = None, limit: int = 100,
                          **_: Any) -> list[dict]:
    ...
    # -- metrics -----------------------------------------------------------
    def _query_prometheus(self, expr: str, start: Any = None, end: Any = None,
                          queryType: str = "range", aggregation: str = "last",
                          **_: Any) -> dict:
    ...
    # -- logs --------------------------------------------------------------
    def _query_loki(self, expr: str, start: Any = None, end: Any = None,
                    limit: int = 50, **_: Any) -> dict:
        ...
        return {
            ...
            "result": [
                {"timestamp": entry.ts.isoformat(), "labels": entry.labels,
                 "line": entry.line}
                for entry in lines
            ],
        }
    ...
    # -- traces ------------------------------------------------------------
    def _query_traces(self, service: str | None = None, query: str | None = None,
                      minDuration: float | None = None, start: Any = None,
                      end: Any = None, limit: int = 20, **_: Any) -> dict:
        ...
        return {
            ...
            "traces": [
                {
                    "traceID": s.trace_id, "spanID": s.span_id, "parentSpanID": s.parent_id,
                    "rootServiceName": s.service, "name": s.name,
        ...
        return {
            ...
            "spans": [
                {"spanID": s.span_id, "parentSpanID": s.parent_id, "name": s.name,
                 "service": s.service, "durationMs": round(s.duration_ms, 2),
                 "attributes": s.attributes, "status": s.status}
    ...
    def _fetch_profile(self, service: str, profileType: str = "process_cpu:cpu:nanoseconds",
                       **_: Any) -> dict:
        """Fetch a flame-graph summary for one service."""
        return {"service": service, "profileType": profileType,
                "top": self.telemetry.profiles.fetch(service)}

    # -- dashboards --------------------------------------------------------
    def _search_dashboards(self, query: str = "", tag: str | list | None = None,
                           limit: int = 20, **_: Any) -> list[dict]:
    ...
    def _generate_deeplink(self, resourceType: str = "dashboard", dashboardUid: str = "",
                           timeRange: dict | None = None, queryParams: dict | None = None,
                           **_: Any) -> dict:
    ...
    # -- annotations -------------------------------------------------------
    def _find_annotations(self, tags: list | None = None, start: Any = None,
                          end: Any = None, limit: int = 50, **_: Any) -> list[dict]:
        ...
        return [
            {"id": a.annotation_id, "time": a.time.isoformat(),
             "timeEnd": a.time_end.isoformat() if a.time_end else None,
             "text": a.text, "tags": a.tags, "dashboardUID": a.dashboard_uid}
            for a in found
        ]

    def _create_annotation(self, text: str, tags: list | None = None,
                           dashboardUID: str | None = None, panelId: int | None = None,
                           time: Any = None, timeEnd: Any = None, **_: Any) -> dict:
        """Write an annotation onto the Grafana timeline."""
        a = self.telemetry.grafana.add_annotation(
            text, list(tags or []), dashboardUID, panelId,
            _parse_time(time, utcnow()),
            _parse_time(timeEnd, None) if timeEnd else None,
        )
        return {"id": a.annotation_id, "message": "Annotation added", "text": a.text,
                "tags": a.tags}
    ...
    def _list_incidents(self, status: str = "active", limit: int = 20, **_: Any) -> list[dict]:
        ...
        return [
            {"incidentID": i.incident_id, "title": i.title, "severity": i.severity,
             "status": i.status, "createdTime": i.created_at.isoformat(),
             "labels": i.labels, "activityCount": len(i.activity)}
            for i in self.telemetry.grafana.incidents.values()
            if status in ("", i.status)
        ][:limit]

    def _create_incident(self, title: str, severity: str = "minor",
                         labels: dict | None = None, **_: Any) -> dict:
        """Declare a Grafana incident."""
        inc = self.telemetry.grafana.create_incident(title, severity, labels)
        return {"incidentID": inc.incident_id, "title": inc.title,
    ...
    def _get_incident(self, incidentID: str, **_: Any) -> dict:
        ...
        return {"incidentID": inc.incident_id, "title": inc.title, "severity": inc.severity,
                "status": inc.status, "labels": inc.labels, "activity": inc.activity}

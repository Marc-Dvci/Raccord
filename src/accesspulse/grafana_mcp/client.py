...
@dataclass(frozen=True)
class Capability:
    """A thing AccessPulse needs to do, and the server tool names that provide it.
...
CAPABILITIES: tuple[Capability, ...] = (
    Capability("list_datasources", ("list_datasources",), True,
    ...
    Capability("search_dashboards", ("search_dashboards",), True,
               "find the dashboard a human should open"),
    Capability("get_dashboard", ("get_dashboard_by_uid",), False),
    Capability("generate_deeplink", ("generate_deeplink",), False,
               "produce a human-reviewable Grafana link"),
    Capability("find_annotations", ("find_annotations", "get_annotations"), False,
               "read deployment and configuration annotations"),
    Capability("create_annotation", ("create_annotation", "add_annotation"), True,
    ...
    Capability("add_activity_to_incident", ("add_activity_to_incident",), False,
...
class RemoteGrafanaMCP(GrafanaMCPClient):
    ...
    async def connect(self) -> None:
        try:
            ...
            raise MCPUnavailable(
                "the 'mcp' package is required for stdio/http transports: "
                "pip install -e '.[cloud]'"
            ) from exc
        ...
        if self.transport == "stdio":
            ...
            env.setdefault("GRAFANA_SERVICE_ACCOUNT_TOKEN",
            ...
            if hasattr(sh, "streamablehttp_client"):        # SDK 1.x
                ...
                ctx = sh.streamable_http_client(
                    settings.mcp_http_url, http_client=http_client
                )

"""Translate AccessPulse capability calls into a real server's tool schemas.

AccessPulse agents call *capabilities*, not tools: `mcp.call("query_prometheus",
...
def _uid_from_labels(labels: dict) -> str:
    ...
    return f"accesspulse-{slo.replace('.', '-')}" if slo else ""
...
def _res_create_annotation(result: Any, args: dict) -> dict:
    ...
    if isinstance(data, dict):
        return {"id": data.get("id"), "message": data.get("message", "Annotation added"),
                "text": args.get("text", ""), "tags": args.get("tags", [])}
...
def _res_search_dashboards(result: Any, args: dict) -> list[dict]:
    ...
    for d in items if isinstance(items, list) else []:
        ...
        out.append({
            ...
            "tags": d.get("tags", []) or [], "url": d.get("url", ""),
...
def _res_create_incident(result: Any, args: dict) -> dict:
    ...
    if isinstance(data, dict):
        ...
        return {"incidentID": inc.get("incidentID") or inc.get("id"),

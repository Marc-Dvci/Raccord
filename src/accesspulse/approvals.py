...
class ApprovalService:
    ...
    def issue(
        ...
        if allowed_roles and role not in allowed_roles:
            self._log("rejected", action.action_id, approver,
        ...
        self._log("issued", action.action_id, approver, token_id=token_id,
    ...
    def redeem(
        ...
        if issued is None:
            self._log("rejected", action.action_id, payload.get("sub", "unknown"),
        ...
        if payload["ah"] != action.action_hash():
            self._log("rejected", action.action_id, issued.approver,
                      reason="action changed after approval")
            raise ApprovalError(
                "the action changed after it was approved (action hash mismatch)"
            )
        if payload["eh"] != evidence_hash:
            self._log("rejected", action.action_id, issued.approver,
                      reason="evidence changed after approval")
            raise ApprovalError(
                "the evidence changed after approval (evidence hash mismatch)"
            )
    ...
    def outstanding(self) -> list[_Issued]:
        now = utcnow()
        return [i for i in self._issued.values()
                if i.redeemed_at is None and i.expires_at > now]

...
class ScopeAgent:
    ...
    def __init__(self, twin: DigitalTwin, registry: PromiseRegistry,
                 assurance: LiveAssurance) -> None:
        self.twin = twin
    ...
    def run(self, incident: Incident, alert: Alert, group: BreachGroup) -> Scope:
        ...
        for promise in self.registry.for_event(
            ...
            if promise.feature is not group.feature:
                continue
            if languages and promise.language not in languages and \
                    promise.feature is not FeatureType.SIGN_LANGUAGE:
                continue
...
class ChangeCorrelationAgent:
    ...
    def run(
        ...
        for change in changes:
            ...
            # change kind priors
            prior = {"config": 0.20, "deployment": 0.18, "provider": 0.12,
                     "routing": 0.12, "manifest": 0.14, "traffic": 0.06}.get(change.kind, 0.05)
            score += prior

            # remediation changes are ours, never a cause
            if change.actor == "accesspulse.remediation":
                score = -1.0
                contradicting.append("this change is AccessPulse's own remediation")
...
TAXONOMY: tuple[Signature, ...] = (
    Signature(FailureClass.INFRA_CLOCK_SOURCE_CHANGE, FeatureType.CAPTIONS,
              ("cap.drift", "sign.sync"), ("resync", "clock", "grandmaster", "pts realigned"),
              ("config",), ("clock-ptp-primary", "clock-ntp-fallback"), weight=1.5,
              statement="A change of timing reference desynchronised the caption chain "
                        "from programme audio."),
    Signature(FailureClass.CAPTION_PROGRESSIVE_DRIFT, FeatureType.CAPTIONS,
              ("cap.drift",), ("resync", "clock", "pts realigned", "offset"),
              ("config", "deployment"), ("capenc-pool-a", "clock-ptp-primary"), weight=1.4,
              statement="Caption timing is drifting progressively behind spoken dialogue."),
    Signature(FailureClass.CAPTION_CLOCK_OFFSET, FeatureType.CAPTIONS,
              ("cap.drift",), ("offset", "clock"), ("config",), ("clock-ptp-primary",),
              weight=0.9, trend_hint="step",
              statement="A fixed timing offset was introduced into the caption path."),
    Signature(FailureClass.CAPTION_ENCODER_FAILURE, FeatureType.CAPTIONS,
              ("cap.availability", "cap.omission_rate"),
              ("worker exited", "sigsegv", "restart", "crash"),
              ("deployment",), ("capenc-pool-a",), weight=1.3,
              statement="Caption encoder workers are failing, producing intermittent cues."),
    Signature(FailureClass.CAPTION_SOURCE_LOSS, FeatureType.CAPTIONS,
              ("cap.availability",), ("source", "no input"), (), ("capsrc-en", "capsrc-fr"),
              weight=1.0,
              statement="The upstream caption source stopped delivering cues."),
    Signature(FailureClass.CAPTION_MANIFEST_OMISSION, FeatureType.CAPTIONS,
              ("cap.availability", "cap.render_success"),
              ("rendition dropped", "track_map", "manifest"),
              ("manifest",), ("manifest-main",), weight=1.3,
              statement="The promised caption track is no longer declared in the manifest."),
    Signature(FailureClass.CAPTION_WRONG_LANGUAGE, FeatureType.CAPTIONS,
              ("cap.wrong_language",), ("track_map", "routing"), ("routing", "config"),
              ("packager-main", "capenc-pool-a"), weight=1.4,
              statement="A routing or track-map change is delivering the wrong language."),
    Signature(FailureClass.CAPTION_RENDER_FAILURE, FeatureType.CAPTIONS,
              ("cap.render_success",), ("renderer", "layout", "canary"),
              ("deployment",), ("pv-ctv-9.4.0", "pv-web-4.12.0"), weight=1.3,
              scope_hint="player_version",
              statement="A player build renders cues off-screen; the data is correct."),
    Signature(FailureClass.CAPTION_WORD_DROP, FeatureType.CAPTIONS,
              ("cap.omission_rate", "cap.semantic"), (), ("provider",), ("capsrc-en",),
              weight=0.9,
              statement="Cues are losing tokens, degrading semantic preservation."),
    Signature(FailureClass.INFRA_PROVIDER_DEGRADATION, FeatureType.CAPTIONS,
              ("cap.drift", "cap.omission_rate", "cap.semantic"), ("provider", "migration"),
              ("provider",), ("capsrc-en", "cdn-primary"), weight=1.2,
              statement="The caption provider is degrading globally."),
    Signature(FailureClass.INFRA_ENCODER_CPU, FeatureType.CAPTIONS,
              ("cap.drift", "cap.first_caption_latency"), ("cpu", "saturation", "queue_depth"),
              ("traffic",), ("capenc-pool-a",), weight=1.2, trend_hint="ramp",
              statement="Encoder saturation is delaying cue emission."),
    Signature(FailureClass.INFRA_CDN_REGIONAL, FeatureType.CAPTIONS,
              ("cap.availability", "sign.availability"), ("origin fetch", "503", "region"),
              ("provider", "routing"), ("cdn-primary", "region-eu-west"), weight=1.3,
              scope_hint="cdn_region",
              statement="One CDN region stopped serving accessibility renditions."),
    Signature(FailureClass.AD_TRACK_OMISSION, FeatureType.AUDIO_DESCRIPTION,
              ("ad.track_present", "ad.selection"), ("rendition dropped", "manifest"),
              ("manifest",), ("manifest-main",), weight=1.4,
              statement="The described audio track is missing from the manifest."),
    Signature(FailureClass.AD_SILENT_SEGMENT, FeatureType.AUDIO_DESCRIPTION,
              ("ad.audio_present",), ("silence", "no audio"), (), ("adsrc-en", "adsrc-fr"),
              weight=1.3,
              statement="The described track is delivered but carries no audible description."),
    Signature(FailureClass.AD_TIMELINE_DRIFT, FeatureType.AUDIO_DESCRIPTION,
              ("ad.drift",), ("clock", "resync"), ("config",), ("clock-ptp-primary",),
              weight=1.2, trend_hint="ramp",
              statement="Descriptions are landing over dialogue instead of in the gaps."),
    Signature(FailureClass.AD_WRONG_LANGUAGE, FeatureType.AUDIO_DESCRIPTION,
              ("ad.language",), ("track_map",), ("routing",), ("packager-main",), weight=1.3,
              statement="The wrong description language is routed onto the track."),
    Signature(FailureClass.AD_LOUDNESS_DEFECT, FeatureType.AUDIO_DESCRIPTION,
              ("ad.loudness",), ("loudness", "mix"), (), ("adsrc-en", "adsrc-fr"),
              weight=1.3,
              statement="The described track is mixed outside the loudness window and is "
                        "masked by programme dialogue."),
    Signature(FailureClass.AD_CHANNEL_LAYOUT, FeatureType.AUDIO_DESCRIPTION,
              ("ad.selection", "ad.audio_present"), ("channel", "layout"),
              ("manifest", "config"), ("packager-main",), weight=1.25,
              statement="The described track is published with a channel layout the "
                        "promised players cannot select."),
    Signature(FailureClass.AD_SELECTION_FAILURE, FeatureType.AUDIO_DESCRIPTION,
              ("ad.selection",), ("assertion", "canary", "build"), ("deployment",),
              ("pv-ctv-9.4.0", "pv-web-4.12.0"), weight=1.35, scope_hint="player_version",
              statement="The audio-track menu lists the described track but selecting it "
                        "fails on one player build."),
    Signature(FailureClass.CAPTION_DUPLICATE, FeatureType.CAPTIONS,
              ("cap.duplicate", "cap.reading_speed"), ("segment published", "manifest"),
              ("manifest", "deployment"), ("packager-main",), weight=1.1,
              statement="The packager is repeating the previous cue, producing "
                        "stuttering captions."),
    Signature(FailureClass.CAPTION_READING_SPEED, FeatureType.CAPTIONS,
              ("cap.reading_speed", "cap.flicker"), (), (), ("capenc-pool-a",), weight=1.05,
              statement="Cue durations are too short to be read at the promised limit."),
    Signature(FailureClass.CAPTION_SPEAKER_CORRUPTION, FeatureType.CAPTIONS,
              ("cap.speaker_accuracy",), (), (), ("capenc-pool-a",), weight=1.2,
              statement="Speaker attribution labels are being swapped between characters."),
    Signature(FailureClass.CAPTION_FLICKER, FeatureType.CAPTIONS,
              ("cap.flicker",), ("canary", "renderer"), ("deployment",),
              ("pv-ctv-9.4.0",), weight=1.2, scope_hint="player_version",
              statement="A player build is tearing cues down and re-rendering them "
                        "within a second."),
    Signature(FailureClass.INFRA_STALE_CONFIG, FeatureType.CAPTIONS,
              ("cap.wrong_language",), ("track-map", "track_map", "partial", "rollout"),
              ("config",), ("capenc-pool-a",), weight=1.15,
              statement="One encoder node is still running an old track map after a "
                        "partial configuration rollout."),
    Signature(FailureClass.INFRA_PACKET_LOSS, FeatureType.CAPTIONS,
              ("cap.availability", "cap.drift"), ("origin fetch", "503", "retransmit"),
              ("routing", "provider"), ("cdn-primary",), weight=1.05,
              scope_hint="cdn_region",
              statement="Transport loss on the contribution link is causing cue gaps "
                        "and timing spread."),
    Signature(FailureClass.SIGN_BLACK_FRAMES, FeatureType.SIGN_LANGUAGE,
              ("sign.black", "sign.availability"), ("interpreter feed degraded",), (),
              ("signsrc-lsf",), weight=1.25,
              statement="The interpreter feed is dropping to black between segments."),
    Signature(FailureClass.SIGN_PIP_OBSTRUCTION, FeatureType.SIGN_LANGUAGE,
              ("sign.visibility",), ("overlay", "branding"), ("deployment",),
              ("pv-web-4.12.0",), weight=1.15,
              statement="Burned-in branding is overlapping the interpreter panel."),
    Signature(FailureClass.SIGN_REGIONAL_DELIVERY, FeatureType.SIGN_LANGUAGE,
              ("sign.availability",), ("origin fetch", "503", "region"),
              ("provider", "routing"), ("cdn-primary", "region-eu-west"), weight=1.3,
              scope_hint="cdn_region",
              statement="The interpreter rendition is not cached in one CDN region."),
    Signature(FailureClass.INFRA_GPU_SATURATION, FeatureType.SIGN_LANGUAGE,
              ("sign.framerate", "sign.frozen"), ("gpu_util", "degraded"), (),
              ("signsrc-lsf",), weight=1.2,
              statement="The interpreter transcode is falling behind a saturated GPU pool."),
    Signature(FailureClass.PLAYER_MISSING_NAME, FeatureType.ACCESSIBLE_PLAYER,
              ("player.accessible_name", "player.screen_reader"),
              ("assertion", "build"), ("deployment",), ("pv-web-4.12.0",), weight=1.15,
              statement="Icon-only playback controls have lost their accessible names."),
    Signature(FailureClass.PLAYER_INACCESSIBLE_ERROR, FeatureType.ACCESSIBLE_PLAYER,
              ("player.screen_reader",), ("assertion", "canary"), ("deployment",),
              ("pv-ctv-9.4.0",), weight=1.05, scope_hint="player_version",
              statement="Playback errors render visually but are never announced to "
                        "assistive technology."),
    Signature(FailureClass.SIGN_FROZEN_FRAMES, FeatureType.SIGN_LANGUAGE,
              ("sign.frozen", "sign.availability"), ("interpreter feed degraded", "freeze"),
              (), ("signsrc-lsf",), weight=1.3,
              statement="The interpreter feed is freezing on repeated frames."),
    Signature(FailureClass.SIGN_LOW_FRAMERATE, FeatureType.SIGN_LANGUAGE,
              ("sign.framerate",), ("fps", "gpu_util", "degraded"), ("deployment",),
              ("signsrc-lsf",), weight=1.3,
              statement="The interpreter feed frame rate has collapsed below readability."),
    Signature(FailureClass.SIGN_CROP_FAILURE, FeatureType.SIGN_LANGUAGE,
              ("sign.visibility",), ("aspect", "crop"), ("config",), ("signsrc-lsf",),
              weight=1.2,
              statement="A framing change is cropping the interpreter's signing space."),
    Signature(FailureClass.SIGN_SYNC_DRIFT, FeatureType.SIGN_LANGUAGE,
              ("sign.sync",), ("clock", "resync"), ("config",), ("clock-ptp-primary",),
              weight=1.2, trend_hint="ramp",
              statement="The interpreter feed has lost synchronisation with programme audio."),
    Signature(FailureClass.INFRA_DEPLOY_REGRESSION, FeatureType.ACCESSIBLE_PLAYER,
              ("player.accessible_name", "player.caption_control", "player.screen_reader",
               "cap.render_success"),
              ("canary", "assertion failed", "build"), ("deployment",),
              ("pv-ctv-9.4.0", "pv-web-4.12.0"), weight=1.5, scope_hint="player_version",
              statement="A player deployment regressed several accessibility behaviours."),
    Signature(FailureClass.PLAYER_KEYBOARD_TRAP, FeatureType.ACCESSIBLE_PLAYER,
              ("player.keyboard", "player.caption_control"), ("focus", "trap", "assertion"),
              ("deployment",), ("pv-web-4.12.0",), weight=1.3,
              statement="Keyboard focus cannot leave the caption dialog."),
    Signature(FailureClass.PLAYER_SCREEN_READER, FeatureType.ACCESSIBLE_PLAYER,
              ("player.screen_reader",), ("live region", "assertion"), ("deployment",),
              ("pv-web-4.12.0",), weight=1.1,
              statement="A screen-reader incompatibility blocks the playback journey."),
    Signature(FailureClass.PLAYER_FOCUS_LOSS, FeatureType.ACCESSIBLE_PLAYER,
              ("player.focus_visible",), ("focus",), ("deployment",), ("pv-web-4.12.0",),
              weight=1.0,
              statement="The visible focus indicator was removed."),
    Signature(FailureClass.PLAYER_REDUCED_MOTION, FeatureType.ACCESSIBLE_PLAYER,
              ("player.reduced_motion",), ("motion", "animation"), ("deployment",),
              ("pv-web-4.12.0",), weight=1.0,
              statement="prefers-reduced-motion is being ignored."),
    Signature(FailureClass.PLAYER_AUTH_FAILURE, FeatureType.ACCESSIBLE_AUTH,
              ("auth.completion",), ("challenge", "a11y_fallback"), ("deployment",),
              ("auth-svc",), weight=1.4,
              statement="A new authentication challenge has no accessible alternative."),
    Signature(FailureClass.PLAYER_PURCHASE_FAILURE, FeatureType.ACCESSIBLE_PURCHASE,
              ("purchase.completion",), ("label", "form"), ("deployment",),
              ("purchase-flow",), weight=1.3,
...
class DiagnosisAgent:
    ...
    def run(
        ...
        log_text = " ".join(
            str(e.payload) for e in evidence
            if e.kind.value in ("loki_query", "grafana_alert")
        ).lower()
        ...
        for sig in TAXONOMY:
            ...
            if sig.log_keywords:
                if kw_hits:
                    ...
                    contradicting.append(
                        f"no log line mentions {list(sig.log_keywords)[:3]}"
                    )
                    score -= 0.15
            ...
            if sig.scope_hint == "player_version" and len(scope.player_versions) in (1, 2) \
                    and len(scope.player_versions) < 5:
                score += 0.25
                supporting.append(
                    f"symptom is confined to build(s) {list(scope.player_versions)}"
                )
        ...
        for i, (sig, posterior, supporting, contradicting) in enumerate(ranked[:4], start=1):
            causal_id = None
            if causal and sig.change_components and \
                    causal[0].change.component in sig.change_components:
            ...
            abstain = (i == 1 and posterior < self.ABSTAIN_BELOW
                       and margin < self.ABSTAIN_MARGIN)
            hypotheses.append(Hypothesis(
                ...
                uncertainty_note="; ".join(supporting + [f"counter: {c}" for c in contradicting]),
                abstained=abstain,
            ))
        return hypotheses
...
class CommunicationAgent:
    ...
    def run(self, incident: Incident, recovered: bool) -> list[Communication]:
        ...
        def add(audience: str, subject: str, body: str, internal: bool = True,
                reading: str = "") -> None:
        ...
        add("operator", f"[{incident.severity.value}] {incident.title}", f"""
...
""")
        ...
        add("accessibility_specialist", f"Accessibility impact: {features}", f"""
...
""")
        ...
        add("technical_director", f"Decision required: {incident.title}", f"""
...
""")
        ...
        add("viewer_support", "Viewer support guidance", f"""
...
""")
        ...
        add("executive", "Accessibility incident summary", f"""
...
def _is_plural(scope: Scope) -> bool:
    return scope.features[0] in (
        FeatureType.CAPTIONS, FeatureType.SUBTITLES, FeatureType.ACCESSIBLE_PLAYER
    )
...
class ReliabilityLearningAgent:
    ...
    def run(
        ...
        proposals.append(
            "add this scenario to the benchmark corpus with its ground-truth class"
        )
        ...
        return PostIncidentReview(
            ...
            verification_complete=bool(incident.assertions) and all(
                a.status.value == "passing" for a in incident.assertions if a.mandatory
            ),

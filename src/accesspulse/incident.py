...
def _p_communicated(inc: Incident) -> list[str]:
    ...
    leaking = [c.communication_id for c in inc.communications
...
class IncidentMachine:
    ...
    def __init__(self, incident: Incident, store: "IncidentStore | None" = None) -> None:
        ...
        if not incident.audit:
            self._append_audit("incident.opened", None, IncidentState.DETECTED,
                               actor="accesspulse.coordinator", detail={})
    ...
    def transition(
        ...
        actor: str = "accesspulse.coordinator",
        **detail,
    ) -> AuditEvent:
        ok, unmet = self.can(target)
        if not ok:
            self._append_audit("transition.rejected", self.incident.state, target, actor,
                               {"unmet": unmet, **detail})
...
class IncidentStore:
    ...
    def append_audit(self, ev: AuditEvent) -> None:
        self._conn.execute(
            ...
            (
                ev.incident_id, ev.seq, ev.at.isoformat(), ev.actor, ev.event,
                ev.from_state.value if ev.from_state else None,
                ev.to_state.value if ev.to_state else None,
                json.dumps(ev.detail, default=str), ev.prev_hash, ev.hash,
            ),
        )
        self._conn.commit()

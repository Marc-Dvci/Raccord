"""AccessPulse command line.

    accesspulse serve                 # run the web product on :8080
    accesspulse hero                  # run the hero incident end to end in the terminal
    accesspulse certify               # run preflight certification
    accesspulse bench --scenarios 1000
    accesspulse mcp                   # show the resolved Grafana MCP tool surface
    accesspulse faults                # list the fault library
"""
...
app = typer.Typer(add_completion=False, help="AccessPulse - Accessible Experience Reliability")
console = Console()
...
def serve(
    ...
    port: int = typer.Option(None, help="defaults to AP_API_PORT"),
    reload: bool = typer.Option(False),
) -> None:
    """Run the AccessPulse web product and API."""
    import uvicorn

    settings = get_settings()
    console.print(Panel.fit(
        f"[bold]AccessPulse[/bold]\n"
        ...
        title="starting", border_style="blue"))
    uvicorn.run("accesspulse.api:app", host=host, port=port or settings.api_port,
...
async def _hero(fault: str, ticks: int, seconds: float, transport: str | None,
                json_out: Path | None) -> None:
    rt = AccessPulseRuntime(mcp_transport=transport, db_prefix="cli")
    await rt.connect()
    console.print(f"[dim]Grafana MCP: {rt.mcp.transport}, "
                  f"{len(rt.mcp.tool_names)} tools advertised[/dim]")

    for _ in range(3):
        rt.tick(15)
    console.print("[green]baseline healthy[/green]: "
                  f"{len(rt.assurance.breaches())} SLOs breaching")
    ...
    console.print(Panel.fit(
        ...
        title="fault injected", border_style="yellow"))
    ...
    t.add_row("diagnosis", f"{_yn(result.diagnosis_correct)} "
                           f"({result.top_posterior:.3f} posterior)")
    if inc:
        t.add_row("state", inc.state.value)
        t.add_row("scope", f"{'/'.join(inc.scope.territories)} · "
                           f"{'/'.join(inc.scope.player_versions)} · "
                           f"{'/'.join(inc.scope.languages)}"
                  if inc.scope else "—")
        t.add_row("scope precision/recall",
                  f"{result.scope_precision:.2f} / {result.scope_recall:.2f}")
        t.add_row("policy", inc.policy_decision.classification.value
                  if inc.policy_decision else "—")
        t.add_row("approval", f"{inc.approval.approver} ({inc.approval.approver_role.value})"
                  if inc.approval else "not required")
    t.add_row("action", result.action_taken or "—")
    t.add_row("verification", f"{result.assertions_passing}/{result.assertions_total} "
                              f"assertions passing")
    t.add_row("recovered", _yn(result.recovered))
    t.add_row("sessions affected / protected",
    ...
    for i, call in enumerate(rt.mcp.call_log, 1):
        console.print(f"  {i:>2}. {call['tool']:<28} "
                      f"{call['duration_ms']:>7.2f} ms  {call['result_bytes']:>6} B")

    if inc:
        for c in inc.communications:
            if c.audience == "public_status":
                console.print(Panel(c.body, title="public status update",
                                    border_style="green"))

    if json_out:
        payload = {
            "result": {k: (v.value if hasattr(v, "value") else v)
                       for k, v in result.__dict__.items() if k != "incident"},
            "incident": inc.model_dump(mode="json") if inc else None,
            "mcp_calls": rt.mcp.call_log,
            "review": (rt.coordinator.reviews[inc.incident_id].model_dump(mode="json")
                       if inc and inc.incident_id in rt.coordinator.reviews else None),
        }
...
async def _certify(transport: str | None) -> None:
    ...
    for gate, row in summary["by_gate"].items():
        t.add_row(gate, str(row["passing"]), str(row["failing"]),
                  str(row["inconclusive"]), str(row["hard"]))
    console.print(t)
...
def mcp(transport: str = typer.Option(None)) -> None:
    """Show the Grafana MCP tool surface and the capabilities AccessPulse resolves."""
...
async def _mcp(transport: str | None) -> None:
    ...
    rt = AccessPulseRuntime(mcp_transport=transport, db_prefix="cli")
    await rt.connect()
    console.print(f"[bold]transport[/bold] {rt.mcp.transport}   "
                  f"[bold]tools[/bold] {len(rt.mcp.tool_names)}")
    t = Table("capability", "required", "resolved tool", "purpose")
    for cap in CAPABILITIES:
        t.add_row(cap.key, "yes" if cap.required else "no",
...
def faults() -> None:
    ...
    for f in FAULT_LIBRARY.values():
        t.add_row(f.fault_id, f.feature.value, f.name, f.failure_class.value,
                  f"{f.difficulty:.2f}")
    console.print(t)

...
Nothing consequential happens in Raccord unless a rule in this module allows
...
POLICY_VERSION = "raccord-policy-2026.08.1"
...
ACTION_CATALOG: dict[ActionType, ActionSpec] = {
    ActionType.SELECT_SYNCHRONIZED_STANDBY: ActionSpec(
        ...
        {
            "raccord_caption_drift_seconds": "falls below 1.5 s within 60 s",
            "raccord_caption_track_available_ratio": "remains >= 0.999",
        },
        "caption_recovery",
    ...
    ActionType.SWITCH_CAPTION_ENCODER_POOL: ActionSpec(
        ...
        ("capenc-pool-b",),
        Role.STREAMING_SRE,
        {"raccord_caption_track_available_ratio": "returns to >= 0.999"},
        "caption_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ActionType.CHANGE_CLOCK_SOURCE: ActionSpec(
        ...
        Role.STREAMING_SRE,
        {
            "raccord_caption_drift_seconds": "falls below 1.5 s",
            "raccord_sign_sync_drift_seconds": "falls below 0.5 s",
        },
        "caption_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ...
    ActionType.RESTORE_KNOWN_GOOD_PLAYER: ActionSpec(
        ...
        Role.STREAMING_SRE,
        {
            "raccord_player_keyboard_completion_ratio": "returns to 1.0",
            "raccord_caption_render_success_ratio": "returns to >= 0.999",
        },
        "player_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ActionType.REPUBLISH_MANIFEST: ActionSpec(
        ...
        ("manifest-main",),
        Role.STREAMING_SRE,
        {"raccord_ad_track_declared_ratio": "returns to 1.0"},
        "manifest_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ...
    ActionType.SWITCH_ALTERNATE_LANGUAGE_SOURCE: ActionSpec(
        ...
        Role.A11Y_SPECIALIST,
        {"raccord_caption_semantic_score": "returns above objective"},
        "caption_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ActionType.RESTART_SIGN_PIPELINE: ActionSpec(
        ...
        Role.A11Y_SPECIALIST,
        {
            "raccord_sign_frozen_frame_ratio": "falls below 0.01",
            "raccord_sign_fps": "returns to >= 45",
        },
        "sign_recovery",
        "auto_rollback_on_verification_failure",
    ),
    ActionType.DISABLE_PLAYER_FEATURE_FLAG: ActionSpec(
        ...
        (FeatureType.ACCESSIBLE_PLAYER, FeatureType.SIGN_LANGUAGE, FeatureType.ACCESSIBLE_AUTH),
        ("flag exists and is runtime-toggleable",),
        ("ctv-9.4.0", "web-4.12.0", "festival-branding-overlay", "bot-challenge"),
        Role.OPERATOR,
        {"raccord_player_reduced_motion_ok_ratio": "returns to 1.0"},
        "player_recovery",
        "auto_rollback_on_verification_failure",
    ),
...
def evaluate(
    ...
    approvals = [
        (rid, o) for rid, o in outcomes if o.classification is PolicyClass.APPROVAL_REQUIRED
    ]
    ...
    if prohibited:
        ...
        roles = ()
        rationale = tuple(f"{rid}: {o.reason}" for rid, o in auto) or ("no rule required approval",)
        violated = ()

...
SUITES: dict[str, tuple[AssertionSpec, ...]] = {
    ...
    "player_recovery": (
        AssertionSpec(
            "keyboard_journey_recovered",
            "Keyboard journey completes again",
            "player.keyboard",
            "original",
        ),
        AssertionSpec(
            "screenreader_journey_recovered",
            "Screen-reader journey completes again",
            "player.screen_reader",
            "original",
        ),
        AssertionSpec(
            "accessible_names_recovered",
            "Controls expose accessible names",
            "player.accessible_name",
            "original",
        ),
        AssertionSpec(
            "caption_render_recovered",
            "Captions render on the affected devices",
            "cap.render_success",
            "original",
            feature=FeatureType.CAPTIONS,
        ),
        AssertionSpec(
            "other_platforms_unregressed",
    ...
    "manifest_recovery": (
        ...
        AssertionSpec(
            "track_selectable",
            "The track can be selected in a real player",
            "ad.selection",
            "original",
            feature=FeatureType.AUDIO_DESCRIPTION,
        ),
        AssertionSpec(
            "caption_tracks_unregressed",
    ...
    "sign_recovery": (
        AssertionSpec(
            "sign_continuity_recovered",
            "Interpreter feed frames are continuous",
            "sign.frozen",
            "original",
            feature=FeatureType.SIGN_LANGUAGE,
        ),
        AssertionSpec(
            "sign_framerate_recovered",
            "Interpreter feed frame rate recovered",
            "sign.framerate",
            "original",
            feature=FeatureType.SIGN_LANGUAGE,
        ),
        AssertionSpec(
            "sign_visibility_recovered",
            "The interpreter is fully visible",
            "sign.visibility",
            "original",
            feature=FeatureType.SIGN_LANGUAGE,
        ),
        AssertionSpec(
            "captions_unregressed",
            "Captions were not disturbed",
            "cap.drift",
            "dependent",
            feature=FeatureType.CAPTIONS,
        ),
    ...
    "delivery_recovery": (
        AssertionSpec(
            ...
            "The rerouted region serves the promise again",
            "cap.availability",
            "original",
        ),
        AssertionSpec(
            "target_region_unregressed",
            "The region absorbing traffic stayed healthy",
            "cap.availability",
            "adjacent",
        ),
        AssertionSpec(
            "sign_availability_recovered",
            "Interpreter feed is delivered again",
            "sign.availability",
            "dependent",
            feature=FeatureType.SIGN_LANGUAGE,
        ),
...
SUITES["description_recovery"] = (
    AssertionSpec(
        "described_audio_audible",
        "Described audio is audible again",
        "ad.audio_present",
        "original",
        feature=FeatureType.AUDIO_DESCRIPTION,
    ),
    AssertionSpec(
        "described_track_declared",
        "The described track is declared",
        "ad.track_present",
        "original",
        feature=FeatureType.AUDIO_DESCRIPTION,
    ),
    AssertionSpec(
        "described_track_selectable",
        "The described track can be selected",
        "ad.selection",
        "original",
        feature=FeatureType.AUDIO_DESCRIPTION,
    ),
    AssertionSpec(
        "description_timing_recovered",
        "Descriptions land in the dialogue gaps",
        "ad.drift",
        "original",
        feature=FeatureType.AUDIO_DESCRIPTION,
        mandatory=False,
    ...
    AssertionSpec(
        "other_territories_unregressed",
        "Unaffected territories are unaffected",
        "ad.track_present",
        "adjacent",
        feature=FeatureType.AUDIO_DESCRIPTION,
    ),
...
SUITES["access_flow_recovery"] = (
    AssertionSpec(
        "auth_completes",
        "Accessible sign-in completes again",
        "auth.completion",
        "original",
    ...
    AssertionSpec(
        "purchase_completes",
        "Accessible purchase completes again",
        "purchase.completion",
        "original",
    ...
    AssertionSpec(
        "player_journeys_unregressed",
        "Playback journeys still complete",
        "player.keyboard",
        "dependent",
        feature=FeatureType.ACCESSIBLE_PLAYER,
    ),
...
def run_suite(
    ...
    for spec in suite:
        ...
        if not observed_values:
            ...
            observed = worst
            status = (
                AssertionStatus.FAILING if slo.breached(worst, tier) else AssertionStatus.PASSING
            )
...
def all_mandatory_passed(assertions: list[VerificationAssertion]) -> bool:
    return all(a.status is AssertionStatus.PASSING for a in assertions if a.mandatory)

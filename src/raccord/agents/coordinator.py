...
REMEDIATION_MAP: dict[FailureClass, tuple[ActionType, str, dict[str, Any]]] = {
    ...
    FailureClass.CAPTION_CLOCK_OFFSET: (ActionType.CHANGE_CLOCK_SOURCE, "clock-ptp-primary", {}),
    FailureClass.CAPTION_ENCODER_FAILURE: (
        ActionType.SWITCH_CAPTION_ENCODER_POOL,
        "capenc-pool-b",
        {},
    ),
    FailureClass.CAPTION_SOURCE_LOSS: (ActionType.REROUTE_CAPTION_PATH, "secondary", {}),
    FailureClass.CAPTION_MANIFEST_OMISSION: (ActionType.REPUBLISH_MANIFEST, "manifest-main", {}),
    FailureClass.INFRA_MALFORMED_MANIFEST: (ActionType.REPUBLISH_MANIFEST, "manifest-main", {}),
    FailureClass.CAPTION_WRONG_LANGUAGE: (ActionType.REROUTE_CAPTION_PATH, "secondary", {}),
    FailureClass.CAPTION_RENDER_FAILURE: (
        ActionType.RESTORE_KNOWN_GOOD_PLAYER,
        "ctv-9.4.0",
        {"from_version": "ctv-9.4.0", "to_version": "ctv-9.3.1"},
    ),
    FailureClass.CAPTION_WORD_DROP: (ActionType.SWITCH_CAPTION_ENCODER_POOL, "capenc-pool-b", {}),
    FailureClass.CAPTION_DUPLICATE: (ActionType.REPUBLISH_MANIFEST, "manifest-main", {}),
    ...
    FailureClass.INFRA_ENCODER_CPU: (ActionType.SWITCH_CAPTION_ENCODER_POOL, "capenc-pool-b", {}),
    FailureClass.INFRA_PROVIDER_DEGRADATION: (
        ActionType.SWITCH_ALTERNATE_LANGUAGE_SOURCE,
        "capsrc-en",
        {"language": "en"},
    ),
    FailureClass.INFRA_STALE_CONFIG: (ActionType.SWITCH_CAPTION_ENCODER_POOL, "capenc-pool-b", {}),
    ...
    FailureClass.AD_TRACK_OMISSION: (ActionType.RESTORE_AUDIO_TRACK, "en-desc", {}),
    FailureClass.AD_SILENT_SEGMENT: (ActionType.RESTORE_AUDIO_TRACK, "en-desc", {}),
    FailureClass.AD_WRONG_LANGUAGE: (
        ActionType.SWITCH_ALTERNATE_LANGUAGE_SOURCE,
        "adsrc-en",
        {"language": "en"},
    ),
    FailureClass.AD_TIMELINE_DRIFT: (ActionType.CHANGE_CLOCK_SOURCE, "clock-ptp-primary", {}),
    FailureClass.AD_LOUDNESS_DEFECT: (ActionType.RESTORE_AUDIO_TRACK, "fr-desc", {}),
    FailureClass.AD_CHANNEL_LAYOUT: (ActionType.REPUBLISH_MANIFEST, "manifest-main", {}),
    FailureClass.AD_SELECTION_FAILURE: (
        ActionType.RESTORE_KNOWN_GOOD_PLAYER,
        "ctv-9.4.0",
        {"from_version": "ctv-9.4.0", "to_version": "ctv-9.3.1"},
    ),
    FailureClass.SIGN_FROZEN_FRAMES: (ActionType.RESTART_SIGN_PIPELINE, "signsrc-lsf", {}),
    FailureClass.SIGN_BLACK_FRAMES: (ActionType.RESTART_SIGN_PIPELINE, "signsrc-lsf", {}),
    FailureClass.SIGN_LOW_FRAMERATE: (ActionType.RESTART_SIGN_PIPELINE, "signsrc-lsf", {}),
    FailureClass.SIGN_CROP_FAILURE: (ActionType.RESTART_SIGN_PIPELINE, "signsrc-lsf", {}),
    FailureClass.SIGN_SYNC_DRIFT: (ActionType.CHANGE_CLOCK_SOURCE, "clock-ptp-primary", {}),
    FailureClass.SIGN_PIP_OBSTRUCTION: (
        ActionType.DISABLE_PLAYER_FEATURE_FLAG,
        "festival-branding-overlay",
        {},
    ),
    FailureClass.INFRA_GPU_SATURATION: (ActionType.RESTART_SIGN_PIPELINE, "signsrc-lsf", {}),
    ...
    FailureClass.PLAYER_AUTH_FAILURE: (ActionType.DISABLE_PLAYER_FEATURE_FLAG, "bot-challenge", {}),
...
class IncidentCoordinator:
    ...
    def open_incident(
        ...
        incident.timings["time_to_detect_s"] = max(0.0, (alert.fired_at - onset).total_seconds())

        with self._step(self.name, "qualify", incident_id):
            machine.transition(
                IncidentState.QUALIFIED, self.name, slo=slo, severity=alert.severity.value
            )
        ...
        with self._step(self.scope_agent.name, "scope", incident_id):
            ...
            machine.transition(
                IncidentState.SCOPED,
                self.scope_agent.name,
    ...
    async def investigate(self, incident: Incident) -> None:
        ...
        machine.transition(
            IncidentState.EVIDENCE_COMPLETE,
    ...
    def diagnose(self, incident: Incident) -> None:
        ...
        with self._step(self.correlation_agent.name, "correlate", incident.incident_id):
            causal = self.correlation_agent.run(incident, incident.changes, incident.scope, onset)
            self.causal[incident.incident_id] = causal

        breached = {e.slo_id for e in self.assurance.last_evaluations if e.breached}
    ...
    async def enrich_reasoning(self, incident: Incident) -> None:
        """Layer Gemini synthesis over facts that deterministic code already settled."""
        if get_settings().reasoning_mode == "offline":
            return
        started = time.perf_counter()
        try:
            incident.reasoning = await adk.synthesise(
                incident,
                self.quality_notes.get(incident.incident_id, []),
                self.causal.get(incident.incident_id, []),
                observer=self.mcp,
            )
        except Exception as exc:
            self.machine(incident.incident_id).note(
                "gemini_synthesis_failed", "gemini_synthesis", error=f"{type(exc).__name__}: {exc}"
            )
            if get_settings().reasoning_fail_open:
                return
            raise
        if incident.reasoning is not None:
            self.telemetry.record_agent_step(
                "gemini_synthesis",
                "synthesise",
                (time.perf_counter() - started) * 1000,
                incident.reasoning.tokens_in,
                incident.reasoning.tokens_out,
                incident.reasoning.model,
            )
            self.machine(incident.incident_id).note(
                "gemini_synthesis_complete",
                "gemini_synthesis",
                model=incident.reasoning.model,
                runtime=incident.reasoning.runtime,
                unknowns=list(incident.reasoning.unknowns),
            )
    ...
    def evaluate_policy(self, incident: Incident, live: bool = True) -> None:
        ...
        if top.abstained:
            ...
            machine.note("escalated_insufficient_evidence", self.name, top_posterior=top.posterior)
            machine.transition(
        ...
        if mapping is None:
            machine.note("no_remediation_mapped", self.name, failure_class=top.failure_class.value)
            machine.transition(
                IncidentState.REJECTED,
                self.name,
        ...
        action = ProposedAction(
            ...
            verification_suite=suite_for(incident.scope.features[0], spec.verification_suite),
        ...
        ctx = PolicyContext(
            ...
            concurrent_actions=tuple(
                ...
                if m.state is IncidentState.ACTION_EXECUTING
                and m.incident.proposed_action
        ...
        machine.transition(
            ...
            required_roles=[r.value for r in incident.policy_decision.required_roles],
    ...
    def reject(self, incident: Incident, actor: str, reason: str) -> None:
        self.machine(incident.incident_id).transition(IncidentState.REJECTED, actor, reason=reason)
    ...
    async def remediate(self, incident: Incident) -> None:
        ...
        with self._step("remediation_agent", "execute", incident.incident_id):
            try:
                incident.action_result = self.executor.execute(incident, incident.proposed_action)
            except ExecutionRefused as exc:
                machine.note("execution_refused", "remediation_agent", error=str(exc))
                machine.transition(IncidentState.REJECTED, "remediation_agent", reason=str(exc))
                return

        text = (
            f"Raccord {incident.incident_id}: approved action "
    ...
    async def verify(self, incident: Incident, settle_seconds: float = 20.0) -> bool:
        ...
        incident.evidence.extend(await self.evidence_agent.verify_recovery(incident, slo))
    ...
    async def communicate(self, incident: Incident, recovered: bool = True) -> None:
        machine = self.machine(incident.incident_id)
        with self._step(self.communication_agent.name, "compose", incident.incident_id):
            deterministic = self.communication_agent.run(incident, recovered)
        actor = self.communication_agent.name
        incident.communications = deterministic
        if get_settings().reasoning_mode == "gemini":
            started = time.perf_counter()
            try:
                generated = await adk.generate_communications(
                    incident, deterministic, observer=self.mcp
                )
            except Exception as exc:
                machine.note(
                    "gemini_communications_failed",
                    "gemini_communications",
                    error=f"{type(exc).__name__}: {exc}",
                )
                if not get_settings().reasoning_fail_open:
                    raise
            else:
                if generated is not None:
                    incident.communications, meta = generated
                    actor = "gemini_communications"
                    self.telemetry.record_agent_step(
                        actor,
                        "compose",
                        (time.perf_counter() - started) * 1000,
                    ...
                    machine.note(
                        "gemini_communications_complete",
                        actor,
                        model=meta["model"],
                        runtime=meta["runtime"],
                    )
        machine.transition(
            IncidentState.COMMUNICATED,
            actor,
    ...
    async def enrich_review(
        self, incident: Incident, review: PostIncidentReview
    ) -> PostIncidentReview:
        """Add Gemini pattern-finding after deterministic closure and scoring."""
        if get_settings().reasoning_mode == "offline":
            return review
        started = time.perf_counter()
        try:
            generated, meta = await adk.analyse_review(incident, review, observer=self.mcp)
        except Exception as exc:
            self.machine(incident.incident_id).note(
                "gemini_learning_failed",
                "gemini_learning",
                error=f"{type(exc).__name__}: {exc}",
            )
            if get_settings().reasoning_fail_open:
                return review
            raise
        self.reviews[incident.incident_id] = generated
        self.telemetry.record_agent_step(
            "gemini_learning",
            "review",
            (time.perf_counter() - started) * 1000,
...
class _StepTimer:
    ...
    def __init__(self, coord: IncidentCoordinator, agent: str, step: str, incident_id: str) -> None:
        self.coord = coord
        self.agent = agent
        self.step = step
...
def publish_metrics(assurance: LiveAssurance, telemetry: TelemetryPlane) -> int:
    ...
    for e in assurance.last_evaluations:
        telemetry.metrics.record(
            "raccord_slo_breached",
            1.0 if e.breached else 0.0,
            {
                "slo": e.slo_id,
                **{
                    k: v
                    for k, v in e.labels.items()
                    if k in ("language", "territory", "platform", "player_version")
                },
            },
            now,
        )
    ...
    for b in assurance.ledger.all():
        telemetry.metrics.record(
            "raccord_error_budget_consumed_ratio", b.consumed_fraction, {"slo": b.slo_id}, now
        )
        count += 1
    # session aggregates
    for obs, _ in assurance.last_reports:
        ...
        telemetry.metrics.record(
            "raccord_sessions_caption_enabled", obs.sessions_with_captions, labels, now
        )
        telemetry.metrics.record(
            "raccord_sessions_description_enabled", obs.sessions_with_description, labels, now
        )
        telemetry.metrics.record(
            "raccord_sessions_sign_enabled", obs.sessions_with_sign, labels, now
        )
        telemetry.metrics.record(
            "raccord_transport_packet_loss_ratio", obs.transport.packet_loss_ratio, labels, now
        )
        telemetry.metrics.record(
            "raccord_encoder_cpu_utilisation", obs.transport.encoder_cpu, labels, now
        )
        telemetry.metrics.record(
            "raccord_gpu_utilisation", obs.transport.gpu_utilisation, labels, now
        )
        count += 6
    return count

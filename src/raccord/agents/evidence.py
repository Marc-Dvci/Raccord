...
class GrafanaEvidenceAgent:
    ...
    def _query_clock(self) -> datetime:
        """Clock used by the selected telemetry backend's query window."""
        simulator = getattr(self.mcp, "sim", None)
        if self.mcp.transport == "stub" and simulator is not None:
            return simulator.wall_clock
        return utcnow()
    ...
    async def _optional(self, capability: str, **arguments: Any) -> Any | None:
        ...
        try:
            ...
            self.telemetry.logs.append(note, {"service": "raccord", "level": "warn"})
            return None
    ...
    async def investigate(
        ...
        # Stub telemetry lives on the accelerated programme clock. Remote
        # telemetry is shifted onto wall time when exported, so its Grafana
        # queries must use real now. Mixing the clocks made fast API runs query
        # a window that ended before every simulated sample existed.
        end = self._query_clock()
        ...
        # 1. the firing alert -------------------------------------------------
        rules = _aslist(
            await self.mcp.call("list_alert_rules", label_selectors={"feature": feature}, limit=50)
        )
        if not rules:
        ...
        rule = next(
            (r for r in firing if r.get("labels", {}).get("slo") == slo),
            firing[0] if firing else None,
        )
        if rule is None:
            raise MCPCallError(
                f"no alert rule advertised for SLO {slo}; the Grafana stack is not "
                "provisioned with the Raccord rule group"
            )
        evidence.append(
            _ev(
                incident.incident_id,
                EvidenceKind.GRAFANA_ALERT,
                "list_alert_rules",
        ...
        evidence.append(
            _ev(
                incident.incident_id,
                EvidenceKind.SESSION_AGGREGATE,
                "query_prometheus",
        ...
        warn_lines = [
            r
            for r in logs.get("result", [])
            if r.get("labels", {}).get("level") in ("warn", "error")
        ]
        evidence.append(
            _ev(
                incident.incident_id,
                EvidenceKind.LOKI_QUERY,
                "query_loki_logs",
        ...
        dashboards = _aslist(
            await self.mcp.call(
                "search_dashboards", query="Incident investigation", tag="raccord", limit=5
            )
        )
        dash_uid = dashboards[0]["uid"] if dashboards else "raccord-incident"
        deep_link = None
        ...
        declared = await self._optional(
            ...
            labels={"slo": slo, "feature": feature, "raccord_incident": incident.incident_id},
        )
    ...
    async def record_action(self, incident: Incident, text: str) -> Evidence | None:
        ...
        for gi in _aslist(listed) if listed is not None else []:
            if gi.get("labels", {}).get("raccord_incident") == incident.incident_id:
                gi_id = gi["incidentID"]
                break
        if gi_id:
        ...
        result = _asdict(
            await self.mcp.call(
                "create_annotation",
                text=text,
                tags=["raccord", "approved-action", incident.incident_id],
                dashboardUID="raccord-incident",
            )
        )
        return _ev(
            incident.incident_id,
            EvidenceKind.ANNOTATION,
            "create_annotation",
            "Approved action written to the Grafana timeline",
            result,
        )
    ...
    async def verify_recovery(
        ...
        annotation = _asdict(
            await self.mcp.call(
                "create_annotation",
                text=f"Raccord: {incident.incident_id} recovered - {metric} back inside "
                f"objective (worst {worst.get('value')})",
                tags=["raccord", "recovery", incident.incident_id],
                dashboardUID="raccord-incident",
            )
        )
        out.append(
            _ev(
                ...
                EvidenceKind.ANNOTATION,
                "create_annotation",
                "Recovery annotation written to Grafana",
                annotation,
            )
        )
        return out
...
def _metric_for_slo(slo: str) -> str:
    ...
    return definition.sli_metric if definition else "raccord_caption_drift_seconds"


def _sessions_metric_for(feature: str) -> str:
    return {
        "captions": "raccord_sessions_caption_enabled",
        "audio_description": "raccord_sessions_description_enabled",
        "sign_language": "raccord_sessions_sign_enabled",
    }.get(feature, "raccord_sessions_caption_enabled")

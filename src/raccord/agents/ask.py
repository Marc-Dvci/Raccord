...
* **Gemini (`RACCORD_REASONING_MODE=gemini`)** answers with the Grafana MCP toolset in
...
_INTENTS: list[tuple[str, str]] = [
    ("ruled_out", r"\b(rule[sd]?\s*out|why not|discount|dismiss|reject|too low|rank(ed)? low)"),
    ("approval", r"\b(approv|authoris|authoriz|who signed|sign-?off|permission|token)"),
    ("verified", r"\b(verif|how do (you|we) know it|is it (really )?fixed|assertion|proof|prove)"),
    (
        "change",
    ...
    ("unknown", r"\b(unknown|uncertain|confiden|not sure|abstain|gap|risk)"),
    (
        "evidence",
...
def answer_offline(incident: Incident, question: str, causal: list | None = None) -> Answer:
    ...
    def done(text: str, cites: list[dict]) -> Answer:
        return Answer(question=question, text=text, plane="offline", evidence=cites, intent=intent)
    ...
    if intent == "ruled_out":
        ...
        return done(
            ...
            if top
            else f"{target.failure_class.value}: posterior {target.posterior:.2f}.",
            cites,
        )
    ...
    if intent == "verified":
        ...
        return done(
            ...
            + (
                ...
                if failed
                else "Nothing failed, so the incident was allowed to close."
            ),
            _cite(
                evidence,
                [
                    ...
                    or "post-action" in (e.summary or "").lower()
                ][:3],
            ),
        )
    ...
    if intent == "change":
        ...
        return done(
            ...
            "among several.",
            _cite(evidence, [e.evidence_id for e in evidence if "annotation" in e.source_tool][:3]),
        )
    ...
    return done(
        ...
        + (
            ...
            if top
            else "No hypothesis has been ranked yet. "
        )
...
async def answer(incident: Incident, question: str, mcp=None, causal: list | None = None) -> Answer:
    ...
    if not question:
        return Answer(
            question=question,
            text="Ask a question about this incident.",
            plane="offline",
            intent="summary",
        )
    ...
    try:
        result = await adk.ask(incident, question, observer=mcp)
    except Exception as exc:  # optionally degrade only when configured to

...
The only component in Raccord permitted to change the production
...
class RemediationExecutor:
    ...
    def execute(self, incident: Incident, action: ProposedAction) -> ActionResult:
        ...
        if decision.classification is PolicyClass.PROHIBITED:
            raise ExecutionRefused("policy prohibits this action: " + "; ".join(decision.rationale))
        ...
        if decision.classification is PolicyClass.APPROVAL_REQUIRED:
            ...
            try:
                self.approvals.redeem(incident.approval.token, action, incident.evidence_hash())
    ...
    def rollback(self, incident: Incident, action: ProposedAction) -> ActionResult:
        ...
        self.sim.caption_encoder_pool = before.get(
            "caption_encoder_pool", self.sim.caption_encoder_pool
        )
        ...
        self.sim.record_change(
            "config",
            action.target,
            f"rollback of {action.action_type.value}",

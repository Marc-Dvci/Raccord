"""Raccord runtime: one object that wires the whole control plane together.
...
from .cloud import CloudEvidenceSink
...
class RaccordRuntime:
    def __init__(
        ...
        self.cloud = CloudEvidenceSink(settings)
        self.assurance = LiveAssurance(
            self.sim, self.registry, event_id, SLOTier.TIER_0_GLOBAL_LIVE
        )
    ...
    def reset(self) -> None:
        ...
        self.registry.reset()
        self.promises = seed_promises(self.registry, self.event_id, start=self.sim.start_wall)
        attach_promises(self.twin, self.promises)
        self.telemetry.clear()
        self.mcp.call_log.clear()
        self.assurance = LiveAssurance(
            self.sim, self.registry, self.event_id, SLOTier.TIER_0_GLOBAL_LIVE
        )
    ...
    def tick(
        ...
        if self.cloud.enabled:
            snapshot["cloud_published"] = self.cloud.publish_tick(self.event_id, snapshot)
        return snapshot
    ...
    def _emit_profiles(self) -> None:
        self.telemetry.profiles.record(
            "raccord-probe-fleet",
            {
                "align_tokens": 41.2,
                "text.embed": 18.7,
                "identify_language": 9.4,
                "simulator.observe": 16.1,
                "assurance.evaluate": 8.3,
                "other": 6.3,
            },
        )
        self.telemetry.profiles.record(
            "raccord-agent",
            {
                "evidence.investigate": 34.8,
                "diagnosis.run": 12.6,
                "verification.run_suite": 39.9,
                "other": 12.7,
            },
        )
    ...
    def inject(self, fault_id: str, scope_override: dict | None = None) -> str:
        ...
        if spec.causal_change:
            self.telemetry.grafana.add_annotation(
                ...
                dashboard_uid="raccord-incident",
        ...
        for change in self.sim.changes[:7]:
            self.telemetry.grafana.add_annotation(
                change.description,
                [change.kind, "change", change.component],
                dashboard_uid="raccord-cockpit",
                at=change.at,
            )
        return af.uid
    ...
    async def run_incident(
        ...
        try:
            ...
            await self.coordinator.enrich_reasoning(incident)
            ...
            if incident.state is IncidentState.REJECTED:
                ...
                if decision.classification is PolicyClass.APPROVAL_REQUIRED:
                    if not auto_approve:
                        return self._result(
                            incident, ground_truth, False, False, "awaiting human approval"
                        )
                    chosen_role = decision.required_roles[0] if decision.required_roles else role
                ...
                if incident.state is IncidentState.VERIFYING:
                    ...
                    if recovered:
                        await self.coordinator.communicate(incident, True)
                    review = self.coordinator.review(incident, ground_truth)
                    review = await self.coordinator.enrich_review(incident, review)
                    if self.cloud.enabled:
                        self.cloud.persist_incident(self.event_id, incident, review)
                else:
    ...
    def _result(self, incident, ground_truth, recovered, rolled_back, error):
        ...
        return ScenarioResult(
            ...
            action_taken=(
                incident.proposed_action.action_type.value if incident.proposed_action else None
            ),
            mcp_calls=len(self.mcp.call_log),
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "error_budget": {
                b.slo_id: round(b.consumed_fraction, 4) for b in self.assurance.ledger.worst(6)
            },
            ...
            "deployment": {
                ...
                "reasoning_runtime": (
                    "agent-engine" if self.settings.agent_engine_resource else "local"
                ),
            },
        }

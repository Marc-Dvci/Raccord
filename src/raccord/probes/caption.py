...
def _cue_token_times(obs: SliceObservation) -> tuple[list[str], list[float], list[str]]:
    ...
    for cue in obs.cues:
        ...
        if not toks:
            continue
        span = min(MAX_TOKEN_SPAN_S, max(MIN_TOKEN_SPAN_S, (cue.end_s - cue.start_s) / len(toks)))
...
def run(obs: SliceObservation, promised_language: str | None = None) -> ProbeReport:
    ...
    report.metrics["raccord_caption_track_available_ratio"] = available
    ...
    if not obs.cues or not ref_tokens:
        ...
        report.metrics["raccord_caption_omission_ratio"] = 1.0 if ref_tokens else 0.0
        return report
    ...
    wrong_language = (
        1.0 if (detected != "unknown" and detected != lang and lang_conf > 0.25) else 0.0
    )
    report.findings.append(
        finding(
            PROBE,
    ...
    report.metrics["raccord_caption_wrong_language_ratio"] = wrong_language
    ...
    report.metrics["raccord_caption_drift_seconds"] = abs(drift)
    ...
    report.findings.append(
        finding(
            ...
            omission,
            unit="ratio",
            confidence=0.9 if enough else 0.4,
            interval=window,
            detail={"unmatched_reference_tokens": len(alignment.unmatched_reference)},
        )
    )
    report.metrics["raccord_caption_omission_ratio"] = omission
    ...
    report.metrics["raccord_caption_semantic_score"] = sem
    ...
    report.metrics["raccord_caption_speaker_accuracy_ratio"] = speaker_acc

    # -- duplicates, reading speed, flicker --------------------------------
    dupes = sum(
        1
        for a, b in zip(obs.cues, obs.cues[1:])
    ...
    report.metrics["raccord_caption_duplicate_ratio"] = dup_ratio

    ok_speed = sum(
        1 for c in obs.cues if text.reading_speed_cps(c.text, c.end_s - c.start_s) <= 20.0
    )
    ...
    report.metrics["raccord_caption_reading_speed_ok_ratio"] = speed_ratio
    ...
    report.metrics["raccord_caption_flicker_ratio"] = flicker
    ...
    report.metrics["raccord_caption_first_latency_seconds"] = first_latency

    # -- rendered output ---------------------------------------------------
    rendered = sum(1 for c in obs.cues if c.rendered) / len(obs.cues)
    render_success = (
        min(rendered, 1.0 if obs.player.captions_rendered else 0.0)
        if obs.player.console_errors
        else rendered
    )
    report.findings.append(
        finding(
    ...
    report.metrics["raccord_caption_render_success_ratio"] = render_success

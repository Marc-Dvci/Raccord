...
def run(obs: SliceObservation, promised_language: str | None = None) -> ProbeReport:
    ...
    report.findings.append(
        finding(
            ...
            confidence=0.99,
            interval=window,
            detail={"manifest_audio_tracks": obs.manifest_audio_tracks},
        )
    )
    report.metrics["raccord_ad_track_declared_ratio"] = declared
    ...
    if not obs.described:
        ...
        report.findings.append(
            finding(
                ...
                VERSION,
                "ad.audio_present",
                0.0 if not insufficient else 0.0,
                unit="ratio",
                confidence=0.9 if not insufficient else 0.5,
                abstained=insufficient,
                data_quality="insufficient" if insufficient else "ok",
                interval=window,
                limitations=("no described window overlapped this observation window",),
            )
        )
        report.metrics["raccord_ad_audio_present_ratio"] = 0.0
        report.metrics["raccord_ad_selection_success_ratio"] = (
    ...
    report.findings.append(
        finding(
            ...
            interval=window,
            detail={
                "windows": len(obs.described),
                "silent_windows": len(obs.described) - audible,
                "silence_floor_dbfs": SILENCE_FLOOR_DBFS,
            },
        )
    )
    report.metrics["raccord_ad_audio_present_ratio"] = audible_ratio
    ...
    report.metrics["raccord_ad_language_match_ratio"] = match
    ...
    report.findings.append(
        finding(
            ...
            unit="s",
            confidence=0.9,
            ci=(min(drifts), max(drifts)),
            interval=window,
            detail={"windows": len(obs.described)},
        )
    )
    report.metrics["raccord_ad_drift_seconds"] = drift
    ...
    report.metrics["raccord_ad_dialogue_masking_ok_ratio"] = masking_ok
    ...
    report.findings.append(
        finding(
            ...
            interval=window,
            detail={
                "target_lufs": [LOUDNESS_MIN, LOUDNESS_MAX],
                "observed": [round(w.loudness_lufs, 1) for w in obs.described],
            },
        )
    )
    report.metrics["raccord_ad_loudness_in_range_ratio"] = loud_ratio

    # -- channel layout ----------------------------------------------------
    layout_ok = sum(1 for w in obs.described if w.channels in (1, 2)) / len(obs.described)
    report.metrics["raccord_ad_channel_layout_ok_ratio"] = layout_ok
    ...
    report.metrics["raccord_ad_selection_success_ratio"] = selection
    ...
    report.metrics["raccord_ad_semantic_coverage_advisory"] = coverage
    if uncovered:
        report.notes.append(

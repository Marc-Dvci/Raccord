...
Nothing in Raccord requires an accelerator.
"""
...
if _HAVE_TRITON:  # pragma: no cover - requires an NVIDIA GPU
    ...
    def substitution_kernel(
        ref_id_ptr,
        hyp_id_ptr,  # int32[n], int32[m]  token identity
        ref_pre_ptr,
        hyp_pre_ptr,  # int32[n], int32[m]  4-char prefix id
        ref_long_ptr,
        hyp_long_ptr,  # int8[n],  int8[m]   len >= 4
        out_ptr,  # float32[n, m]
        n,
        m,
        SUB_COST: tl.constexpr,
        NEAR_COST: tl.constexpr,
    ...
    def wavefront_kernel(
        ...
        diag = tl.load(d_ptr + (i - 1) * stride + (j - 1), mask=mask, other=0.0) + tl.load(
            sub_ptr + (i - 1) * m + (j - 1), mask=mask, other=0.0
        )
...
def _codes(reference: list[str], hypothesis: list[str]) -> tuple[np.ndarray, ...]:
    ...
    def code(tokens: list[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        return (
            np.fromiter(
                (ids.setdefault(t, len(ids)) for t in tokens), dtype=np.int32, count=len(tokens)
            ),
            np.fromiter(
                (pres.setdefault(t[:4], len(pres)) for t in tokens),
                dtype=np.int32,
                count=len(tokens),
            ),

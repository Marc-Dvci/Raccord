...
async function refreshState() {
  ...
  state.demoMode = s.deployment.demo_mode;
  $('#deployment-mode').textContent = state.demoMode
    ? 'isolated simulator · demo approvals'
    : 'production · authenticated approvals';
...
function renderIncident(data) {
  ...
  const reasoning = inc.reasoning;
  $('#reasoning-body').replaceChildren(reasoning
    ? el('div', {},
      el('h3', {}, 'Gemini evidence synthesis'),
      el('p', {}, reasoning.narrative),
      el('p', { class: 'uncertainty' }, reasoning.confidence_statement),
      el('h4', {}, 'Supporting evidence'),
      el('ul', {}, ...(reasoning.supporting.length ? reasoning.supporting : ['none'])
        .map((item) => el('li', {}, item))),
      el('h4', {}, 'Contradictions and unknowns'),
      el('ul', {}, ...[...reasoning.contradicting, ...reasoning.unknowns]
        .map((item) => el('li', {}, item))),
      el('p', {}, el('strong', {}, 'Next evidence: '),
        reasoning.recommended_next_evidence || 'none'),
      el('p', { class: 'hint' },
        `${reasoning.model} · ${reasoning.runtime} · ` +
        `${reasoning.tokens_in + reasoning.tokens_out} tokens`))
    : el('p', { class: 'hint' },
      'Offline mode: ranked hypotheses remain deterministic and reproducible.'));
  ...
  if (review) {
    ...
    if (review.learning_narrative) {
      $('#review-body').append(
        el('h3', {}, 'Gemini learning analysis (non-authoritative)'),
        el('p', {}, review.learning_narrative),
        el('ul', {}, ...review.proposed_experiments.map((m) => el('li', {}, m))),
        el('p', { class: 'hint' },
          `${review.reasoning_model} · ${review.reasoning_runtime}`));
    }
  }
...
function renderApproval(data) {
  ...
  if (inc.approval) {
    ...
    const approverInput = el('input', {
      ...
      disabled: state.demoMode ? null : 'disabled',
    });
    ...
    body.append(
      el('p', { class: 'hint' }, state.demoMode
        ? 'Demo identity: this isolated simulator does not assert a real operator login.'
        : 'Identity comes from the trusted IAP header; the request cannot claim another user.'),
...
async function loadBenchmark() {
  try {
    ...
    $('#bench-body').replaceChildren(el('p', { class: 'hint' },
      `No results yet (${err.message}). Run: raccord bench --scenarios 1000`));
  }
}
...
function wire() {
  ...
  $('#btn-judge-demo').addEventListener('click', async (ev) => {
    busy(ev.target, true);
    say('Resetting the event and running the complete governed recovery …');
    try {
      ...
      say(data.error
        ? `Judge run stopped: ${data.error}`
        : `Verified recovery: ${data.assertions[0]}/${data.assertions[1]} assertions, ` +
          `${data.mcp_calls} Grafana MCP calls, unsafe action ${data.unsafe_action ? 'yes' : 'no'}.`,
        data.error ? 'warn' : 'ok');
  ...
  $('#btn-run-loop').addEventListener('click', async (ev) => {
    ...
    try {
      const data = await api(`/api/incident/run?auto_approve=${state.demoMode}`, { method: 'POST' });
      renderIncident(data);
      say(data.error

...
The SLO catalogue in `raccord/slo.py` is the single source of truth. Running
this script regenerates:

  observability/grafana/dashboards/*.json
  observability/grafana/provisioning/alerting/raccord-rules.yml
...
from raccord.contracts import SLOTier  # noqa: E402
from raccord.slo import (  # noqa: E402
    ALL_SLOS,
    Comparator,
    SLODefinition,
)
...
PROM = {"type": "prometheus", "uid": "raccord-prom"}
LOKI = {"type": "loki", "uid": "raccord-loki"}
TEMPO = {"type": "tempo", "uid": "raccord-tempo"}
PYRO = {"type": "grafana-pyroscope-datasource", "uid": "raccord-pyro"}
...
def timeseries(
    ...
    legend: str = "{{language}} / {{territory}} / {{player_version}}",
    description: str = "",
) -> dict:
    return {
        "id": pid,
...
def table(pid: int, title: str, expr: str, grid: dict, description: str = "") -> dict:
    return {
        ...
        "gridPos": grid,
        "targets": [
            {"refId": "A", "expr": expr, "format": "table", "instant": True, "datasource": PROM}
        ],
...
def traces(pid: int, title: str, query: str, grid: dict) -> dict:
    return {
        ...
        "datasource": TEMPO,
        "gridPos": grid,
        "targets": [{"refId": "A", "queryType": "traceql", "query": query, "datasource": TEMPO}],
    }
            ...
            "list": [
                {
                    ...
                    "target": {
                        "type": "tags",
                        "matchAny": True,
                        "tags": ["raccord", "approved-action", "recovery"],
                    },
                },
                {
...
TEMPLATE_VARS = [
    {
        ...
        "multi": True,
        "label": "Language",
        "query": {"query": "label_values(raccord_caption_drift_seconds, language)"},
    },
    {
        "name": "territory",
        "type": "query",
        ...
        "multi": True,
        "label": "Territory",
        "query": {"query": "label_values(raccord_caption_drift_seconds, territory)"},
    },
    {
        "name": "player_version",
        "type": "query",
        ...
        "multi": True,
        "label": "Player build",
        "query": {"query": "label_values(raccord_caption_drift_seconds, player_version)"},
    },
]
...
def executive_dashboard() -> dict:
    ...
    p.append(
        stat(
            2,
            "Accessibility SLO attainment",
            "1 - avg(raccord_slo_breached)",
            "percentunit",
            _grid(0, 3, 5, 5),
            [
    ...
    p.append(
        stat(
            3,
            "Worst error budget consumed",
            "max(raccord_error_budget_consumed_ratio)",
            "percentunit",
            _grid(5, 3, 5, 5),
            [
    ...
    p.append(
        stat(
            4,
            "Accessibility-enabled sessions protected",
            "sum(raccord_sessions_caption_enabled) + "
            "sum(raccord_sessions_description_enabled) + "
            "sum(raccord_sessions_sign_enabled)",
            "short",
            _grid(10, 3, 5, 5),
        )
    )
    p.append(
        stat(
            5,
            "Incidents open",
            "count(raccord_slo_breached > 0) or vector(0)",
            "short",
            _grid(15, 3, 4, 5),
    ...
    p.append(
        stat(
            ...
            "avg(raccord_time_to_recovery_seconds)",
            "s",
            _grid(19, 3, 5, 5),
            [
                {"color": "green", "value": None},
    ...
    p.append(
        timeseries(
            7,
            "Error budget consumed by SLO",
            "raccord_error_budget_consumed_ratio",
            "percentunit",
            _grid(0, 8, 12, 8),
            legend="{{slo}}",
        )
    ...
    p.append(
        table(
            8,
            "Breaching SLO / slice matrix",
            "raccord_slo_breached > 0",
            _grid(12, 8, 12, 8),
    ...
    p.append(
        timeseries(
            9,
            "Sessions with an accessibility feature enabled",
            "sum by (territory) (raccord_sessions_caption_enabled)",
            "short",
            _grid(0, 16, 12, 7),
            legend="{{territory}}",
    ...
    p.append(
        timeseries(
            10,
            "Agent cost per incident",
            "sum(raccord_agent_cost_usd)",
            "currencyUSD",
            _grid(12, 16, 12, 7),
            legend="{{model}}",
        )
    ...
    return dashboard(
        "raccord-exec",
        "Raccord / Executive accessibility reliability",
        ["raccord", "executive", "slo"],
        p,
        "Accessibility SLO attainment, error budget, protected sessions and "
        "restoration performance for the whole event portfolio.",
        "30s",
    )
...
def cockpit_dashboard() -> dict:
    ...
    p.append(
        text_panel(
            ...
            (
                ...
                "changes; purple annotations are Raccord approved actions and recoveries."
            ),
            _grid(0, 0, 24, 3),
        )
    )
    p.append(
        stat(
            2,
            "Caption drift (worst slice)",
            f"max(raccord_caption_drift_seconds{SEL})",
            "s",
            _grid(0, 3, 4, 5),
            [
                {"color": "green", "value": None},
    ...
    p.append(
        stat(
            3,
            "Caption availability (worst slice)",
            f"min(raccord_caption_track_available_ratio{SEL})",
            "percentunit",
            _grid(4, 3, 4, 5),
    ...
    p.append(
        stat(
            ...
            "min(raccord_ad_audio_present_ratio)",
            "percentunit",
            _grid(8, 3, 4, 5),
    ...
    p.append(
        stat(
            5,
            "Interpreter feed frame rate",
            "min(raccord_sign_fps)",
            "none",
            _grid(12, 3, 4, 5),
    ...
    p.append(
        stat(
            6,
            "Keyboard journeys completing",
            "min(raccord_player_keyboard_completion_ratio)",
            "percentunit",
            _grid(16, 3, 4, 5),
    ...
    p.append(
        stat(
            7,
            "Screen-reader journeys completing",
            "min(raccord_player_screenreader_completion_ratio)",
            "percentunit",
            _grid(20, 3, 4, 5),
            [{"color": "red", "value": None}, {"color": "green", "value": 1}],
        )
    ...
    p.append(
        timeseries(
            8,
            "Caption drift by slice",
            f"raccord_caption_drift_seconds{SEL}",
            "s",
            _grid(0, 8, 12, 9),
    ...
    p.append(table(9, "Territory x platform health", "raccord_slo_breached", _grid(12, 8, 12, 9)))
    p.append(
        timeseries(
            10,
            "Accessibility-enabled sessions in affected slices",
            f"raccord_sessions_caption_enabled{SEL}",
            "short",
            _grid(0, 17, 8, 8),
        )
    )
    p.append(
        timeseries(
            11,
            "Encoder CPU / GPU utilisation",
            "raccord_encoder_cpu_utilisation or raccord_gpu_utilisation",
            "percentunit",
            _grid(8, 17, 8, 8),
        )
    )
    ...
    return dashboard(
        "raccord-cockpit",
        "Raccord / Live event command centre",
        ["raccord", "live", "event"],
        p,
        "What every audience slice is receiving right now, against what was promised.",
        "10s",
        TEMPLATE_VARS,
    )
...
def incident_dashboard() -> dict:
    ...
    p.append(
        text_panel(
            ...
            (
                "The evidence the Raccord agents retrieved through the Grafana MCP server, "
    ...
    p.append(
        timeseries(
            INCIDENT_SLI_PANEL_ID,
            "1-3. Breached SLI over the incident window",
            f"raccord_caption_drift_seconds{SEL}",
            "s",
            _grid(0, 3, 12, 8),
            [{"color": "green", "value": None}, {"color": "red", "value": 1.5}],
    ...
    p.append(
        timeseries(
            3,
            "4. Audience impact (k-anonymised aggregates)",
            f"raccord_sessions_caption_enabled{SEL}",
            "short",
            _grid(12, 3, 12, 8),
            description="query_prometheus",
    ...
    p.append(
        logs(
            4,
            "6. Component logs for the implicated pool",
            '{service=~"capenc-.*|clock-.*"}',
            _grid(0, 11, 12, 9),
        )
    )
    ...
    p.append(
        timeseries(
            ...
            _grid(0, 20, 12, 8),
            [{"color": "green", "value": None}, {"color": "red", "value": 1.5}],
            description="Post-action series read back through MCP before the incident may close.",
        )
    )
    p.append(
        table(
            ...
            "raccord_verification_assertion_status",
            _grid(12, 20, 12, 8),
        )
    )
    return dashboard(
        "raccord-incident",
        "Raccord / Incident investigation",
        ["raccord", "incident", "captions"],
        p,
        "Evidence retrieved through Grafana MCP during an accessibility "
        "incident, plus the post-action verification.",
        "10s",
        TEMPLATE_VARS,
    )
...
def quality_dashboard() -> dict:
    ...
    p.append(
        stat(
            2,
            "Probe abstention rate",
            "avg(raccord_probe_abstained_ratio)",
            "percentunit",
            _grid(0, 3, 6, 5),
        )
    )
    ...
    p.append(
        stat(
            4, "Probe fleet latency p95", "max(raccord_probe_duration_ms)", "ms", _grid(12, 3, 6, 5)
        )
    )
    p.append(
        stat(
            5,
            "Slices evaluated per sweep",
            "max(raccord_slices_evaluated)",
            "short",
            _grid(18, 3, 6, 5),
        )
    )
    ...
    p.append(flamegraph(8, "Probe fleet CPU profile", _grid(0, 16, 12, 9), "raccord-probe-fleet"))
    p.append(flamegraph(9, "Agent CPU profile", _grid(12, 16, 12, 9), "raccord-agent"))
    return dashboard(
        "raccord-quality",
        "Raccord / Probe and model quality",
        ["raccord", "model", "quality"],
        p,
        "Precision, calibration, abstention and cost of the measurement layer itself.",
        "30s",
    )
...
def agent_dashboard() -> dict:
    ...
    p.append(
        text_panel(
            1,
            "Agent and MCP observability",
            (
                "Raccord observes the agent it builds. Every agent step is a span, every "
    ...
    p.append(
        stat(
            2, "MCP calls this incident", "sum(raccord_mcp_calls_total)", "short", _grid(0, 3, 6, 5)
        )
    )
    p.append(
        stat(3, "MCP p95 latency", "max(raccord_mcp_call_duration_ms)", "ms", _grid(6, 3, 6, 5))
    )
    p.append(
        stat(
            4,
            "MCP errors",
            'sum(raccord_mcp_calls_total{status="error"}) or vector(0)',
            "short",
            _grid(12, 3, 6, 5),
            [{"color": "green", "value": None}, {"color": "red", "value": 1}],
        )
    ...
    p.append(
        stat(5, "Agent cost", "sum(raccord_agent_cost_usd)", "currencyUSD", _grid(18, 3, 6, 5))
    )
    p.append(
        timeseries(
            6,
            "MCP call latency by tool",
            "raccord_mcp_call_duration_ms",
            "ms",
            _grid(0, 8, 12, 8),
            legend="{{tool}}",
        )
    ...
    p.append(
        timeseries(
            7,
            "Agent step duration",
            "raccord_agent_step_duration_ms",
            "ms",
            _grid(12, 8, 12, 8),
            legend="{{agent}}",
        )
    ...
    p.append(
        traces(
            ...
            '{ resource.service.name = "raccord-agent" }',
            _grid(0, 16, 12, 9),
        )
    )
    p.append(
        timeseries(
            9,
            "Tokens in / out",
            "raccord_agent_tokens_in or raccord_agent_tokens_out",
            "short",
            _grid(12, 16, 12, 9),
            legend="{{agent}}",
        )
    ...
    return dashboard(
        "raccord-agent",
        "Raccord / Agent and MCP observability",
        ["raccord", "agent", "mcp", "ai-observability"],
        p,
        "Traces, tool selection, latency, token usage and cost for the "
        "Raccord agents and their Grafana MCP interactions.",
        "10s",
    )
        ...
        rules.append(
            {
                ...
                "annotations": {
                    ...
                    "__dashboardUid__": "raccord-incident",
...
def main() -> int:
    ...
    rules_path = ALERT_DIR / "raccord-rules.yml"
    rules_content = (
        "# GENERATED by tools/generate_grafana_assets.py from raccord/slo.py.\n"
    ...
    if not args.check:
        ...
        print(
            f"wrote {len(alert_rules()['groups'][0]['rules'])} alert rules to "

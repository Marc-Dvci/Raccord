...
from raccord.probes.accelerated import (
...
from raccord.probes.align import _token_cost, monotonic_dtw_path
...
def _corpus(rng: random.Random, n: int, m: int, vocab: list[str]):
    return ([rng.choice(vocab) for _ in range(n)], [rng.choice(vocab) for _ in range(m)])
...
def test_backend_matches_the_reference_on_random_input(backend):
    ...
    for _ in range(40):
        ...
        assert _same(align_tokens_reference(reference, hypothesis), fn(reference, hypothesis))
...
def test_substitution_matrix_matches_the_scalar_cost_function():
    ...
    expected = np.array(
        [[_token_cost(r, h) for h in hypothesis] for r in reference], dtype=np.float32
    )
...
def test_dispatcher_short_circuits_small_inputs_to_the_reference():
    ...
    assert _same(
        align_tokens_reference(reference, hypothesis),

...
from raccord.policy import ACTION_CATALOG, PolicyContext, evaluate
...
def _action(action_type=ActionType.SELECT_SYNCHRONIZED_STANDBY, target="capenc-pool-b"):
    return ProposedAction(
        action_id="act-1", incident_id="inc-1", action_type=action_type, target=target
    )
...
def test_public_communication_always_needs_the_support_lead():
    decision = evaluate(
        ...
        PolicyContext(
            tier=SLOTier.TIER_3_CATALOG, live=False, scope=_scope(), operator_roles=(Role.OPERATOR,)
        ),
    )
    assert decision.classification is PolicyClass.APPROVAL_REQUIRED
...
def test_every_catalogued_action_declares_a_verification_suite():
    from raccord.verification import SUITES
